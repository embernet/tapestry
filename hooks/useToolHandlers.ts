...
export const useToolHandlers = ({
    ...
    const handleAiToolSelect = useCallback((toolId: string) => {
        ...
        if (toolId === 'chat') {
            ...
            // Special handling for 'expand' which doesn't have a single base key
            if (toolId === 'expand') {
                let prompt = '';
                if (selectedElementId) {
            ...
            // Standard handling for other tools (summarise, connect, critique)
            const promptKey = `ai:${toolId}`;
            const promptTemplate = promptStore.get(promptKey);

            if (promptTemplate) {
                setChatDraftMessage(promptTemplate);
                panelState.setIsChatPanelOpen(true);
            }
        }
