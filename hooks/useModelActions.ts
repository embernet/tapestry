    ...
    return useMemo(() => {
        ...
        return {
            ...
            addElement: (data) => {
                ...
                console.log('[DEBUG] addElement called', { name: data.name, currentCount: elementsRef.current.length });
                elementsRef.current = [...elementsRef.current, newElement];
                setElements(prev => {
                    console.log('[DEBUG] setElements updater', { prevCount: prev.length, newName: newElement.name });
                    const next = [...prev, newElement];
                    // Sync ref to ensure subsequent synchronous actions (like addRelationship) see the updated list
            ...
            updateElement: (name, data) => {
                ...
                setElements(prev => {
                    const next = prev.map(e => e.id === element.id ? updatedElement : e);
                    elementsRef.current = next;
                    return next;
                });
                return true;
            ...
            deleteElement: (name) => {
                ...
                // Update refs immediately for consistency in batch operations
                setElements(prev => {
                    const next = prev.filter(f => f.id !== element.id);
                    elementsRef.current = next;
                    return next;
                });
                setRelationships(prev => {
                    const next = prev.filter(r => r.source !== element.id && r.target !== element.id);
            ...
            setRelationshipAttribute: (sourceName, targetName, key, value) => {
                ...
                setRelationships(prev => {
                    const next = prev.map(r => r.id === rel.id ? updated : r);
                    relationshipsRef.current = next;
                    return next;
                });
                return true;
