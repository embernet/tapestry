...
export const useGraphLayout = ({ elementsRef, relationshipsRef, setElements, graphCanvasRef }: UseGraphLayoutProps) => {
    ...
    const handleStaticLayout = useCallback(() => {
        ...
        // Automatically zoom to fit after layout
        setTimeout(() => {
            if (graphCanvasRef.current && graphCanvasRef.current.zoomToFit) {
                graphCanvasRef.current.zoomToFit();
            }
        }, 100);

    }, [layoutParams, elementsRef, relationshipsRef, setElements, graphCanvasRef]);
