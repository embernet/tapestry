...
export const ContextMenus: React.FC<ContextMenusProps> = (props) => {
    ...
    const selectedElement = contextMenu ? elements.find(e => e.id === contextMenu.elementId) : null;
    let linkUrl = '';
    if (selectedElement && selectedElement.attributes) {
        const key = Object.keys(selectedElement.attributes).find(k => k.toLowerCase() === 'url');
        if (key) linkUrl = selectedElement.attributes[key];
    }
        ...
        <>
            {contextMenu && (
                <ContextMenu
                    ...
                    linkUrl={linkUrl}
                    onAddLink={() => { props.onAddLink(contextMenu.elementId); }}
                    onEditLink={() => { props.onEditLink(contextMenu.elementId); }}
                    onOpenLink={() => { props.onOpenLink(contextMenu.elementId); }}
