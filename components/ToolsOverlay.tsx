...
import { Element, Relationship, ColorScheme, CustomStrategyTool, NodeShape, KanbanBoard, Script } from '../types';
...
interface ToolsOverlayProps {
    ...
    // Scripts
    handleCreateScript: (name: string, code: string) => string;
    scripts?: Script[];
                ...
                {isToolVisible('scripts') && (
                    <ScriptsToolbar
                        onOpenEditor={() => {
                            ...
                            if (panelState.setScriptPanelMode) {
                                panelState.setScriptPanelMode('edit');
                            }
                            tools.setActiveTool(null);
                        }}
                        onLoadExample={(code) => {
                            const id = props.handleCreateScript(`Example ${new Date().toLocaleTimeString()}`, code);
                            if (id && panelState.setActiveScriptId) {
                                panelState.setActiveScriptId(id);
                            }
                            panelState.setIsScriptPanelOpen(true);
                            if (panelState.setScriptPanelMode) {
                                panelState.setScriptPanelMode('run');
                            }
                            tools.setActiveTool(null);
                        }}
                        onOpenScript={(id) => {
                            if (panelState.setActiveScriptId) {
                                panelState.setActiveScriptId(id);
                            }
                            panelState.setIsScriptPanelOpen(true);
                            tools.setActiveTool(null);
