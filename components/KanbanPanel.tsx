...
import { Element, ModelActions, KanbanBoard } from '../types';
...
interface KanbanPanelProps {
    ...
    onNodeContextMenu: (e: React.MouseEvent, elementId: string, boardId?: string) => void;
    allBoards: KanbanBoard[];
    onSelectBoard: (boardId: string) => void;
}

const KANBAN_ORDER_ATTR_PREFIX = 'kanbanOrder_';

export const KanbanPanel: React.FC<KanbanPanelProps> = ({
    elements, modelActions, onClose, selectedElementId, onNodeClick, isDarkMode, board, onUpdateBoard, onNodeContextMenu, allBoards, onSelectBoard
}) => {
    // State for dynamic columns (excluding Unassigned which is always first)
    // We use board.columns directly, but keep local input state
    ...
    const handleStartEditing = (col: string) => {
    ...
    const handleRenameColumn = () => {
        ...
        // 1. Update all items in this column
        const items = groupedData[oldName] || [];
        items.forEach(el => {
            modelActions.setElementAttribute(el.name, board.attributeKey, newName);
        });

        // 2. Update board columns
        const newCols = board.columns.map(c => c === oldName ? newName : c);
        onUpdateBoard({
            ...board,
            columns: newCols,
    ...
    const allColumns = useMemo(() => ['Unassigned', ...board.columns], [board.columns]);
    ...
    const getOrder = (el: Element): number => {
        const orderKey = `${KANBAN_ORDER_ATTR_PREFIX}${board.id}`;
        const val = parseFloat(el.attributes?.[orderKey] || '0');
    ...
    const groupedData = useMemo(() => {
        ...
        elements.forEach(el => {
            let status = el.attributes?.[board.attributeKey];
        ...
        return groups;
    }, [elements, board.columns, board.attributeKey, allColumns, board.id]);
    ...
    const moveElementToColumn = (elementId: string, targetCol: string, toTop: boolean = false) => {
        ...
        const orderKey = `${KANBAN_ORDER_ATTR_PREFIX}${board.id}`;
        modelActions.setElementAttribute(elementId, orderKey, newOrder.toString());
        ...
        if (targetCol === 'Unassigned') {
            ...
            modelActions.setElementAttribute(elName, board.attributeKey, 'Unassigned');
        } else {
            modelActions.setElementAttribute(elName, board.attributeKey, targetCol);
        }
    };

    const updateCardPosition = (elementId: string, colName: string, order: number) => {
        const orderKey = `${KANBAN_ORDER_ATTR_PREFIX}${board.id}`;
        modelActions.setElementAttribute(elementId, orderKey, order.toString());
        const elName = elements.find(e => e.id === elementId)?.name || '';
        if (colName === 'Unassigned') {
            modelActions.setElementAttribute(elName, board.attributeKey, 'Unassigned');
        } else {
            modelActions.setElementAttribute(elName, board.attributeKey, colName);
        }
    };
    ...
    const handleColumnReorder = (targetColId: string) => {
        ...
        const sourceIdx = currentCols.indexOf(sourceCol);
        const targetIdx = currentCols.indexOf(targetColId);

        if (sourceIdx !== -1 && targetIdx !== -1) {
            const newCols = [...currentCols];
            newCols.splice(sourceIdx, 1);
            newCols.splice(targetIdx, 0, sourceCol);
            onUpdateBoard({ ...board, columns: newCols, updatedAt: new Date().toISOString() });
        }
    };
    ...
    const handleAddColumn = () => {
        if (newColumnName.trim() && !board.columns.includes(newColumnName.trim())) {
            onUpdateBoard({ ...board, columns: [...board.columns, newColumnName.trim()], updatedAt: new Date().toISOString() });
            setNewColumnName('');
    ...
    const handleDeleteColumn = (colName: string) => {
        if (confirm(`Delete column "${colName}"? Items will return to Unassigned.`)) {
            ...
            (groupedData[colName] || []).forEach(el => {
                modelActions.deleteElementAttribute(el.name, board.attributeKey);
            });
            onUpdateBoard({ ...board, columns: board.columns.filter(c => c !== colName), updatedAt: new Date().toISOString() });
        }
    };
            ...
            <div className={`p-4 border-b flex justify-between items-center ${headerBgClass}`}>
                <div className="flex items-center gap-2">
                    <select
                        ...
                        className={`text-xl font-bold bg-transparent border-none focus:ring-0 cursor-pointer ${textClass}`}
                    >
                        {allBoards.map(b => (
                            <option key={b.id} value={b.id} className={isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}>
                                {b.name}
                            </option>
                        ))}
                    </select>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {editingColumn === col ? (
                                        <input
                                            ...
                                            className={`text-sm font-bold px-1 rounded shadow-sm border focus:outline-none w-32 ${isDarkMode ? 'bg-gray-700 text-white border-blue-500' : 'bg-white text-gray-900 border-blue-500'}`}
                                            onClick={(e) => e.stopPropagation()}
                                            onMouseDown={(e) => e.stopPropagation()}
                                        />
                                    ) : (
                                        <span
                                            className={`${textClass} ${!isUnassigned ? 'cursor-text hover:bg-gray-200 dark:hover:bg-gray-700 px-1 rounded -ml-1 transition-colors' : ''}`}
                                            onClick={(e) => {
                                                if (!isUnassigned) {
                                                    e.stopPropagation();
                                                    handleStartEditing(col);
                                                }
                                            }}
                                            title={!isUnassigned ? "Click to rename" : ""}
                                        >
                                            {col}
                                        </span>
                                    )}
                            ...
                            >
                                {items.map((el, idx) => (
                                    <div
                                        ...
                                        onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); onNodeContextMenu(e, el.id, board.id); }}
