...
import { AboutModal, PatternGalleryModal, ConflictResolutionModal, CreateModelModal, SaveAsModal, OpenModelModal, ChangelogModal, ExitConfirmationModal } from './ModalComponents';
...
interface AppModalsProps {
    ...
    handleImportCsv: (newElements: Element[], newRelationships: Relationship[], mode: 'merge' | 'replace') => void;
...
export const AppModals: React.FC<AppModalsProps> = (props) => {
    ...
    return (
        <>
            ...
            {persistence.isOpenModelModalOpen && <OpenModelModal models={persistence.modelsIndex} onLoad={persistence.handleLoadModel} onClose={() => persistence.setIsOpenModelModalOpen(false)} onTriggerCreate={() => { persistence.setIsOpenModelModalOpen(false); persistence.setIsCreateModelModalOpen(true); }} onDelete={persistence.handleDeleteModel} onDeleteAll={persistence.handleDeleteAllModels} />}
            {persistence.pendingImport && (
                <ConflictResolutionModal localMetadata={persistence.pendingImport.localMetadata} diskMetadata={persistence.pendingImport.diskMetadata} localData={persistence.pendingImport.localData} diskData={persistence.pendingImport.diskData} onCancel={() => persistence.setPendingImport(null)} onChooseLocal={() => { persistence.handleLoadModel(persistence.pendingImport!.localMetadata.id); persistence.setPendingImport(null); }} onChooseDisk={() => { persistence.handleOverwriteLocal(persistence.pendingImport!.diskData, persistence.pendingImport!.diskMetadata); persistence.setPendingImport(null); }} />
            )}
            ...
            {persistence.isExitModalOpen && (
                <ExitConfirmationModal
                    onSaveAndExit={persistence.handleExitSave}
                    onDiscardAndExit={persistence.handleExitDiscard}
                    onCancel={persistence.handleExitCancel || (() => persistence.setIsExitModalOpen(false))}
                    isDarkMode={isDarkMode}
                    title={persistence.pendingUnsavedAction === 'createNew' ? "Save current model?" : "Unsaved Changes"}
                    message={persistence.pendingUnsavedAction === 'createNew'
                        ? "You have unsaved changes. Do you want to save your current model before creating a new one?"
                        : "You have unsaved changes in your current model. How would you like to proceed?"}
                    saveLabel={persistence.pendingUnsavedAction === 'createNew' ? "Save & Continue" : "Save & Exit"}
                    discardLabel={persistence.pendingUnsavedAction === 'createNew' ? "Discard & Create New" : "Discard Changes & Exit"}
                />
            )}
        </>
    );
