...
const NodeShapePanel: React.FC<LayoutToolbarProps & { onClose: () => void }> = ({
    ...
    const SHAPES: { value: NodeShape, label: string, icon: React.ReactNode, separator?: boolean }[] = [
        {
            value: 'rectangle',
            label: 'Containing Rectangle',
        ...
        {
            value: 'containing_hexagon',
            label: 'Containing Hexagon',
            icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M2,12 L5,4 L19,4 L22,12 L19,20 L5,20 Z" /></svg>,
            separator: true
        },
        {
            value: 'circle',
        ...
        },
        {
            value: 'hexagon',
            label: 'Hexagon',
            icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M12 2l9 5v10l-9 5l-9-5V7z" /></svg>
        },
        {
            value: 'square',
            label: 'Square',
            icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><rect x="4" y="4" width="16" height="16" rx="2" /></svg>
        }
    ];
    ...
    return (
        ...
        >
            ...
            <div className="p-1 flex flex-col">
                {SHAPES.map(shape => (
                    <React.Fragment key={shape.value}>
                        <button
                        ...
                        {shape.separator && <div className={`h-px mx-2 my-1 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`} />}
                    </React.Fragment>
                ))}
            </div>
        </div>
