...
export const FilterContent: React.FC<FilterContentProps> = ({
    ...
    onTagFilterChange, onDateFilterChange, onNodeFilterChange, hiddenElementIds = [], onUnhide, onUnhideAll, isDarkMode
}) => {
                ...
                <div className={`px-6 py-4 border-b ${sectionBorderClass}`}>
                    ...
                    >
                        <div className="flex items-center space-x-2">
                            <h3 className={`font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>Hidden Items ({hiddenElementIds.length})</h3>
                        </div>
                        <div className="flex items-center space-x-2">
                            <button
                            ...
                            <svg
                                ...
                                className={`h-5 w-5 ${subTextClass} transform transition-transform ${isHiddenExpanded ? 'rotate-180' : ''}`}
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                            >
                    ...
                    {isHiddenExpanded && (
                        <div className="mt-4 space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                            {hiddenElementIds.map(id => {
                                ...
                                return (
                                    <div key={id} className={`flex items-center justify-between p-2 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                                        <span className={`text-sm ${textClass}`}>{element?.name || 'Unknown Element'}</span>
                                        <button
                                            onClick={() => onUnhide(id)}
