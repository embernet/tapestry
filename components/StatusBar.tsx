...
export const StatusBar: React.FC<StatusBarProps> = ({
    ...
    // Storage Usage
    const { usedKB, percent: storagePercent, totalKB } = useStorageUsage();
    ...
    return (
        <div className={`h-8 w-full flex items-center px-4 text-xs select-none z-[500] flex-shrink-0 ${bgClass}`}>
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
                ...
                {/* Storage Usage */}
                <div
                    className="flex items-center gap-2 cursor-help px-2 py-0.5"
                    title={`Local Storage: ${usedKB}KB / ${totalKB}KB Used (${storagePercent}%)`}
                >
                    <div className="flex flex-col w-16 gap-0.5">
                        <div className="flex justify-between text-[9px] leading-none opacity-70 font-mono">
                            <span>{usedKB}KB</span>
                        ...
                        <div className="w-full h-1.5 bg-gray-300 dark:bg-gray-700 rounded-full overflow-hidden">
                            <div
                                className={`h-full transition-all duration-500 ${storagePercent > 90 ? 'bg-red-500' :
                                        storagePercent > 75 ? 'bg-yellow-500' : 'bg-green-500'
                                    }`}
