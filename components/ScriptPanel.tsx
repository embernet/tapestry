...
interface ScriptPanelProps {
    ...
    onCreateScript: (name: string, initialCode?: string) => string | void;
...
export const ScriptPanel: React.FC<ScriptPanelProps> = ({
    ...
    const [isScriptListOpen, setIsScriptListOpen] = useState(false);
    const [lastSavedCode, setLastSavedCode] = useState(DEFAULT_SCRIPT);
    ...
    // Window State
    const { position, setPosition, handleMouseDown: handleDragMouseDown, isDragging } = usePanelDrag({
        initialPosition: { x: Math.max(100, window.innerWidth - 650), y: 100 },
    ...
    // Initial size adjustment for run mode
    useEffect(() => {
        if (mode === 'run') {
            // Smaller default size for run mode if not manually set? 
            // Or just let it be. But maybe we want to collapse the height if it was huge?
            // For now, let's keep size independent but maybe we want to enforce min constraints.
            if (size.height > 400) setSize(prev => ({ ...prev, height: 400 }));
            if (size.width > 450) setSize(prev => ({ ...prev, width: 450 }));
        } else {
            // Restore or default for edit mode
            if (size.width < 600) setSize(prev => ({ ...prev, width: 650 }));
            if (size.height < 500) setSize(prev => ({ ...prev, height: 600 }));
        }
    }, [mode]);
    ...
    useEffect(() => {
        if (activeScriptId) {
            ...
            if (script) {
                setCode(script.code);
                setLastSavedCode(script.code);
            }
        } else {
            // Reset if no script active (new script)
            // If checking 'activeScriptId' is null, it might be a transient state or explicit new
            // We should probably clear code if explicitly null?
            // Actually, keep current code if we just created it.
        }
    ...
    const handleManualSave = () => {
        if (!activeScriptId) {
            ...
            if (!name) return;
            const newId = onCreateScript(name, code);
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            // Drag logic handled by usePanelDrag hook
            if (isResizing) {
    ...
    const initRuntime = () => {
        ...
        const ctx = engine.createContext(toolRegistry, {
            ...
            progressBar: {
                ...
                update: (p, msg) => {
                    // console.log('DEBUG: ProgressBar.update()', p, msg);
                    setProgress(p);
    ...
    const handleCreateNew = () => {
        ...
        setLastSavedCode(DEFAULT_SCRIPT);
    };
    ...
    return (
        <div
            className={`fixed rounded-lg shadow-2xl border ${bgClass} ${borderClass} flex flex-col overflow-hidden`}
            style={{
                left: position.x,
                top: position.y,
                width: size.width,
        ...
        >
                ...
                <div className="flex items-center gap-2">
                    {mode === 'edit' && (
                        <div className="relative">
                            <button
                                onClick={() => setIsScriptListOpen(!isScriptListOpen)}
                                className={`p-1 rounded hover:bg-black/10 transition-colors ${isDarkMode ? 'text-gray-300 hover:bg-white/10' : 'text-gray-600'}`}
                                title="Open Script"
                            >
                            ...
                            {isScriptListOpen && (
                                <div className={`absolute top-full left-0 mt-1 w-48 rounded shadow-xl border z-50 max-h-60 overflow-y-auto ${isDarkMode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-200'}`}>
                                    <div className={`p-2 text-xs font-bold border-b ${isDarkMode ? 'border-gray-700 text-gray-400' : 'border-gray-100 text-gray-500'}`}>
                                        CUSTOM SCRIPTS
                                    </div>
                                    <button
                                        onClick={() => {
                                        ...
                                        className={`w-full text-left px-3 py-2 text-xs flex items-center gap-2 ${isDarkMode ? 'hover:bg-gray-700 text-green-400' : 'hover:bg-gray-50 text-green-600'}`}
                                    >
                                        <span className="font-bold">+ New Script</span>
                                    </button>
                                    {scripts.map(s => (
                                        <button
                                            key={s.id}
                                            ...
                                            className={`w-full text-left px-3 py-2 text-xs truncate ${activeScriptId === s.id ? (isDarkMode ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-600') : (isDarkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-50')}`}
                                        >
                                            {s.name}
                                        </button>
                                    ))}
                                    {scripts.length === 0 && (
                                        <div className="p-3 text-center text-xs opacity-50 italic">No custom scripts</div>
                                    )}
                                </div>
                            )}
                        </div>
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider text-pink-500">
                        {activeScriptId && scripts.find(s => s.id === activeScriptId)
                            ? scripts.find(s => s.id === activeScriptId)?.name
                            : (code ? "Unsaved Script" : "New Script")}
                    </span>
                    ...
                    {mode === 'edit' ? (
                        <>
                            <button
                                ...
                                className={`px-2 py-0.5 text-[10px] rounded border font-bold transition-all ${code !== lastSavedCode
                                    ? (isDarkMode ? 'bg-blue-600 border-blue-500 text-white hover:bg-blue-500' : 'bg-blue-500 border-blue-600 text-white hover:bg-blue-600')
                                    : (isDarkMode ? 'border-gray-700 text-gray-600 opacity-50 cursor-not-allowed' : 'border-gray-200 text-gray-300 opacity-50 cursor-not-allowed')
                                    }`}
                                title={code !== lastSavedCode ? "Save changes" : "No changes to save"}
                            >
                                SAVE
                            </button>
                            <button
                                onClick={() => setMode('run')}
                                className={`px-2 py-0.5 text-[10px] rounded border ${isDarkMode ? 'border-pink-700 text-pink-400 hover:bg-pink-900' : 'border-pink-200 text-pink-600 hover:bg-pink-50'}`}
                            >
                                Switch to Run View
                            </button>
                        </>
                        ...
                        <button
                            ...
                            className={`px-2 py-0.5 text-[10px] rounded border ${isDarkMode ? 'border-gray-600 text-gray-300 hover:bg-gray-700' : 'border-gray-300 text-gray-600 hover:bg-gray-100'}`}
                        >
                            Edit Script
                        </button>
                    )}
            ...
            <div className={`p-2 flex gap-2 border-b items-center ${isDarkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
                ...
                <button
                    ...
                    className={`p-1.5 rounded border transition-colors ${executionState === 'running' ? 'bg-green-600 text-white' : 'text-green-500 border-green-500 hover:bg-green-500 hover:text-white disabled:opacity-50'} ${mode === 'run' ? 'flex-grow flex justify-center py-2' : ''}`}
                    title="Run Script"
                >
                    {mode === 'run' && <span className="mr-2 font-bold uppercase">Run Script</span>}
            ...
            {/* Main Flex Area */}
            <div className={`flex relative overflow-hidden ${mode === 'edit' ? 'flex-grow' : 'hidden'}`}>
            ...
            {/* Console Resize Handle (only in Edit mode) */}
            {mode === 'edit' && (
                <div
            ...
            <div
                ...
                style={mode === 'run' ? { flexGrow: 1 } : { height: consoleHeight }}
            >
                ...
                <div className="flex-grow min-h-0 overflow-y-auto p-2 font-mono text-[10px] space-y-1 custom-scrollbar">
                    {errorMessage && (
            ...
            {/* Status Bar */}
            <div className={`h-6 flex-shrink-0 border-t flex items-center px-2 gap-4 text-[10px] select-none ${isDarkMode ? 'bg-gray-900 border-gray-700 text-gray-400' : 'bg-gray-100 border-gray-200 text-gray-500'}`}>
                <div className="font-mono opacity-70">Lines: {lineNumbers.length}</div>

                {isProgressBarVisible && (
                    <div className="flex-grow flex items-center gap-2 min-w-0 animate-fade-in">
                        <span className="font-bold text-blue-500">Progress:</span>
                        <div className={`flex-grow h-1.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-gray-700' : 'bg-gray-300'}`}>
                            <div
                                className="h-full bg-blue-500 transition-all duration-300 ease-out"
                                style={{ width: `${Math.max(0, Math.min(100, progress))}%` }}
                            ></div>
                        </div>
                        <span className="w-8 text-right font-mono">{Math.round(progress)}%</span>
                        <span className="truncate max-w-[150px] opacity-75 hidden sm:inline border-l ml-1 pl-2 border-gray-500/30" title={progressMessage}>
                            {progressMessage}
                        </span>
                    </div>
                )}
                {!isProgressBarVisible && <div className="flex-grow"></div>}

                <div className="w-2"></div> {/* Spacer for resize handle */}
            </div>
