...
const GraphCanvas = forwardRef<GraphCanvasRef, GraphCanvasProps>(({
  ...
  useEffect(() => {
    ...
    const getShapeSize = (nodeShape: NodeShape) => {
      ...
      if (nodeShape === 'hexagon') return { w: 60, h: 52 };
      if (nodeShape === 'square') return { w: 50, h: 50 };
      return { w: NODE_MAX_WIDTH, h: 80 };
    };

    const currentShapeSize = getShapeSize(nodeShape as NodeShape);
    const isCompact = nodeShape === 'circle' || nodeShape === 'point' || nodeShape === 'diamond' || nodeShape === 'hexagon' || nodeShape === 'square';
    ...
    if (nodeShape === 'oval') {
      ...
      const r = 30; // approx width/2
      const h = 26; // approx height/2, total height 52
      // Pointy sides hexagon: (-r, 0), (-r/2, -h), (r/2, -h), (r, 0), (r/2, h), (-r/2, h)
      // Actually, standard hexagon points for width W=60 (r=30) and height H=52 (h=26)
      // Points: (-30,0), (-15,-26), (15,-26), (30,0), (15,26), (-15,26)
      node.append('polygon')
        .attr('points', '-30,0 -15,-26 15,-26 30,0 15,26 -15,26');
      ...
      node.append('rect')
        ...
        .attr('y', -20)
        .attr('rx', 4).attr('ry', 4);
    } else if (nodeShape === 'containing_hexagon') {
      node.append('polygon');
    } else {
      node.append('rect')
    ...
    node.select(nodeShape === 'oval' ? 'ellipse' : (nodeShape === 'diamond' ? 'rect' : (nodeShape === 'hexagon' ? 'polygon' : (nodeShape === 'square' ? 'rect' : (nodeShape === 'containing_hexagon' ? 'polygon' : (isCompact ? 'circle' : 'rect'))))))
      .attr('fill', (d: any) => {
    ...
    if (isCompact) {
      const offsetX = nodeShape === 'diamond' ? 20 : (nodeShape === 'circle' ? 35 : (nodeShape === 'hexagon' ? 35 : (nodeShape === 'square' ? 25 : 12)));
    ...
    node.each(function (d: any) {
      ...
      if (isCompact) {
        ...
        if (foDiv) {
          ...
          if (nodeShape === 'oval') {
            ...
            const w = width;
            const h = height;
            // Points: (-w/2, 0), (-w/4, -h/2), (w/4, -h/2), (w/2, 0), (w/4, h/2), (-w/4, h/2)
            const points = `${-w / 2},0 ${-w / 4},${-h / 2} ${w / 4},${-h / 2} ${w / 2},0 ${w / 4},${h / 2} ${-w / 4},${h / 2}`;
            nodeElement.select('polygon').attr('points', points);
          } else {
      ...
      // --- Link Icon Logic ---
      const url = d.attributes?.url || d.attributes?.URL; // Case insensitive check handled by checking both or normalizing earlier? Plan said case-insensitive.
      // Let's just check 'url' as per convention, or maybe I should check keys case-insensitively?
      // simpler to just check 'url' property if I stick to that convention.
      // But let's check keys just in case.
      let linkUrl = '';
      if (d.attributes) {
        const key = Object.keys(d.attributes).find(k => k.toLowerCase() === 'url');
        if (key) linkUrl = d.attributes[key];
      }

      const linkIconSize = 16;
      const linkIconPadding = 4;

      // Remove existing link group to avoid duplicates on re-render
      nodeElement.selectAll('.link-icon-group').remove();

      if (linkUrl) {
        const linkGroup = nodeElement.append('g')
          .attr('class', 'link-icon-group')
          .attr('transform', `translate(${width / 2 - linkIconSize / 2 - linkIconPadding}, ${-height / 2 + linkIconSize / 2 + linkIconPadding})`) // Top right corner? No, standard coord system for node is dependent on shape. 
          // Rect is centered at 0,0. top right is w/2, -h/2.
          // So we want it at w/2 - size, -h/2.
          // Actually, let's put it at top right corner, but INSIDE.
          // x: width/2 - linkIconSize - padding
          // y: -height/2 + padding
          .style('cursor', 'pointer')
          .on('click', (e: any) => {
            e.stopPropagation();
        ...
        // Add an invisible rect for easier clicking
        linkGroup.append('rect')
          .attr('width', linkIconSize)
          .attr('height', linkIconSize)
          .attr('fill', 'transparent');

        // External Link Icon (feather-icons style)
        linkGroup.append('path')
          .attr('d', 'M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6M15 3h6v6M10 14L21 3')
          .attr('fill', 'none')
          .attr('stroke', isDarkMode ? '#60a5fa' : '#2563eb') // Blue-400 / Blue-600
          .attr('stroke-width', '2')
          .attr('stroke-linecap', 'round')
          .attr('stroke-linejoin', 'round')
          .attr('transform', 'scale(0.6)'); // Scale down to fit in 16x16 roughly or adjust path coordinates

        // The path above is for 24x24 viewBox.
        // Let's perform a scale transform to fit into our desired size (e.g. 16px).
        // 16/24 = 0.66.
        // But simpler is to allow it to be 24x24 and just shift position.
        // Let's just scale it.

        linkGroup.append('title').text(linkUrl);
      }
