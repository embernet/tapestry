...
const AppHeader: React.FC<AppHeaderProps> = ({
    ...
    return (
        <div id="app-header-container" className={`w-full relative z-[600] flex flex-col items-start transition-colors duration-300 border-b ${headerBg}`}>
            <div className="flex items-center space-x-2 p-2">
                ...
                <div className="relative" ref={mainMenuRef}>
                    ...
                    >
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'} transition-transform ${isMainMenuOpen || isSystemMenuOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    {isMainMenuOpen && (
                        <div className={`absolute top-full left-0 mt-2 w-64 border rounded-lg shadow-xl py-2 flex flex-col z-50 text-sm animate-fade-in-down max-h-[80vh] overflow-y-auto ${menuBg}`}>
                            ...
                            <button onClick={() => { onOpenCsvTool(); setIsMainMenuOpen(false); }} className={`w-full text-left px-4 py-2 flex items-center gap-3 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} ${menuItemHover}`}>
                            ...
                            {[
                                { id: 'ai', label: 'AI Assistant', color: 'text-purple-400' },
                            ...
                            {[
                                ...
                                { label: 'Sunburst', state: panelState.isSunburstPanelOpen, toggle: () => { panelState.setIsSunburstPanelOpen((p: boolean) => !p); if (panelState.isSunburstPanelOpen) panelState.setSunburstState((prev: any) => ({ ...prev, active: false })); else panelState.setSunburstState((prev: any) => ({ ...prev, active: true })); }, icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-500 hover:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg> },
                ...
                <button onClick={onToggleTheme} title={isDarkMode ? "Switch to Light Mode" : "Switch to Dark Mode"} className={`p-2 rounded-md transition ${textClass}`}>
                    {isDarkMode ? (
                ...
                <button onClick={onExit} title="Exit to Start Screen" className={`p-2 rounded-md transition ${textClass} hover:text-red-500`}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
