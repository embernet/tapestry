...
interface ContextMenuProps {
    ...
    // New Props for Link Management
    linkUrl?: string; // If present, shows Open/Edit, else Add
    onAddLink?: () => void;
    onEditLink?: () => void;
    onOpenLink?: () => void;
}
...
export const ContextMenu: React.FC<ContextMenuProps> = ({
    ...
    linkUrl, onAddLink, onEditLink, onOpenLink
}) => {
    ...
    return (
        <div ref={ref} className="fixed bg-gray-800 border border-gray-600 rounded shadow-xl z-[9999] py-1 w-56" style={style}>
            ...
            {linkUrl ? (
                <>
                    <button onClick={() => { onOpenLink?.(); onClose(); }} className="block w-full text-left px-4 py-2 text-sm text-gray-200 hover:bg-blue-600 hover:text-white">
                        Open Link
                    </button>
                    <button onClick={() => { onEditLink?.(); onClose(); }} className="block w-full text-left px-4 py-2 text-sm text-gray-200 hover:bg-blue-600 hover:text-white">
                        Edit Link
                    </button>
                </>
            ) : (
                <button onClick={() => { onAddLink?.(); onClose(); }} className="block w-full text-left px-4 py-2 text-sm text-gray-200 hover:bg-blue-600 hover:text-white">
                    Add Link
                </button>
            )}
