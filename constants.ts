...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: '2025-12-20',
    changes: [
      'TSCRIPT model improvements report',
      'TSCRIPT system improvements report',
      'AI Insight generation from model',
      'Improved multi-step AI automation'
    ]
  },
  {
    version: '0.6.2',
    date: '2025-12-16',
    changes: [
      'Bug Fix: AI Expand tool now correctly loads prompts'
    ]
  },
  {
    version: '0.6.1',
...
export const AVAILABLE_AI_TOOLS = [
  ...
  },
  {
    id: 'settings',
    name: 'Settings',
    description: 'Opens the application settings modal. Can open specific tabs: "general", "ai_settings", "ai_prompts", "ai_tools", "tool_prompts".'
  }
];
...
export const CORE_TOOL_IDS = [
  ...
  'readDocument', 'createDocument', 'updateDocument',
  'getCurrentDate', 'searchNodes', 'openTool'
];
...
export const DEFAULT_SYSTEM_PROMPT_CONFIG: SystemPromptConfig = {
  ...
  userContext: "",
  responseStyle: "",
  enabledTools: [...AVAILABLE_AI_TOOLS.map(t => t.id), ...CORE_TOOL_IDS],
...
4. Console
   - Log(message="...") -> prints message to the script console

5. ProgressBar
   - Show() -> displays the progress bar
   - Update(percentage=0..100, message="...") -> updates progress
   - Hide() -> hides the progress bar
`;

export const DEFAULT_SNIPPETS: ScriptSnippet[] = [
  {
    id: 'model_improvements_report',
    name: 'Model Improvements Report',
    description: 'Generates a detailed report suggesting improvements to the graph model.',
    isSystem: false,
    code: `# Model Improvements Report
# This script analyzes the graph structure (completeness, connectivity)
# and calls the AI to generate a REPORT suggesting improvements.

# 1. Create Report Document
doc_id = markdown.create_doc(title="Model Assessment Report", content="# Model Assessment Report\\n\\nAnalyzing graph structure...\\n")

# 2. Build Graph Context
nodes = graph.get_all_nodes()
context_str = "Graph Content:\\n"
...
for n in nodes:
    context_str = context_str + "- Node: " + n.name + "\\n"
    # Get connections
    conns = graph.get_connections(id=n.id)
    for c in conns:
        if c.isSource:
            context_str = context_str + "  -> " + c.label + " -> " + c.neighbor.name + "\\n"

    idx = idx + 1
    pct = (idx / nodes.length) * 100
    ProgressBar.Update(pct, "Analyzing: " + n.name)
    sleep(0.05)

# 3. Call AI
ProgressBar.Update(100, "Sending to AI...")
prompt = context_str + "\\n\\nBased on the above graph structure representing a system or knowledge base, please suggest 3-5 concrete improvements to the *graph model itself*. Look for missing connections, isolated nodes, or vague terminology. Provide the output as a Markdown report."
Console.Log("Sending to AI...")
response = ai.prompt(text=prompt)

# 4. Append to Document
markdown.append_text(doc=doc_id, text="## Model Suggestions\\n" + response)
markdown.open_doc(id=doc_id)
ProgressBar.Hide()
Console.Log("Done. Report opened.")`
  },

  {
    id: 'system_improvements_report',
    name: 'System Improvements Report',
    description: 'Generates a detailed report suggesting real-world system improvements.',
    isSystem: false,
    code: `# System Improvements Report
# This script analyzes the real-world system described by the graph
# and calls the AI to generate a REPORT suggesting improvements to that system.

# 1. Create Report Document
doc_id = markdown.create_doc(title="System Improvements Report", content="# System Improvements Report\\n\\nAnalyzing system content...\\n")

# 2. Build Graph Context
nodes = graph.get_all_nodes()
context_str = "System Description (Nodes and Relationships):\\n"

for n in nodes:
    context_str = context_str + "- Component: " + n.name + "\\n"
    # Get connections
    conns = graph.get_connections(id=n.id)
    for c in conns:
        if c.isSource:
...
# 3. Call AI
prompt = context_str + "\\n\\nThe above describes a system (business, technical, or conceptual). Please suggest 3-5 concrete improvements to the *system itself* (e.g. efficiency, reliability, new features, cost reduction). Focus on the real-world entity, not the graph data structure. Provide the output as a Markdown report."
Console.Log("Sending to AI...")
response = ai.prompt(text=prompt)

# 4. Append to Document
markdown.append_text(doc=doc_id, text="## Recommended System Improvements\\n" + response)
markdown.open_doc(id=doc_id)
Console.Log("Done. Report opened.")`
  },
...
nodes = graph.get_all_nodes()
Console.Log("Generating report for " + nodes.length + " nodes...")
ProgressBar.Show()
current_index = 0

for n in nodes:
    report = report + "## " + n.name + "\\n"
    Console.Log("Processing node: " + n.name)
    ...
    # Progress Bar Update
    # Percentage calculation approximation
    current_index = current_index + 1
    pct = (current_index / nodes.length) * 100
    ProgressBar.Update(pct, "Processing node: " + n.name)
...
# 1. Create Report Document
doc_id = markdown.create_doc(title="Model Improvement Report", content="# Model Improvement Report\\n\\nNote: This report contains suggestions for improving the model.\\n\\nAnalyzing graph structure...\\n")
...
Console.Log("Done. Report opened.")`
  ...
  {
    id: 'model_improvement_questions',
    name: 'Model Improvement Questions',
    description: 'Generates questions to help improve the quality and scope of the model.',
    code: `# Model Improvement Questions
# This script generates questions to help expand and refine the model.

# 1. Create Document
doc_id = markdown.create_doc(title="Model Improvement Questions", content="# Model Improvement Questions\\n\\nAnalyzing model structure...\\n")
...
# 3. Call AI
ProgressBar.Update(100, "Generating questions with AI...")
prompt = context_str + "\\n\\nBased on the above graph structure, generate a list of thought-provoking questions designed to help the user improve the quality and scope of the model. The goal is to make the model more complete and ensure it covers important aspects. Focus on the modeling aspect: are there missing connections? Are some definitions vague? Are there alternative perspectives missing? Provide the output as a Markdown list."
Console.Log("Generating questions...")
response = ai.prompt(text=prompt)

# 4. Append to Document
markdown.append_text(doc=doc_id, text="## Questions for Model Improvement\\n" + response)
markdown.open_doc(id=doc_id)
ProgressBar.Hide()
Console.Log("Done.")`
  },
  ...
  {
    id: 'system_improvement_questions',
    name: 'System Improvement Questions',
    description: 'Generates questions to help think about improving the real-world system.',
    code: `# System Improvement Questions
# This script generates questions to help think about improving the system itself.

# 1. Create Document
doc_id = markdown.create_doc(title="System Improvement Questions", content="# System Improvement Questions\\n\\nAnalyzing system...\\n")

# 2. Build Context
nodes = graph.get_all_nodes()
context_str = "System Description:\\n"
...
# 3. Call AI
ProgressBar.Update(100, "Generating questions with AI...")
prompt = context_str + "\\n\\nThe above describes a system. Ask questions about the system from the perspective of improving it. DO NOT make suggestions, only ask questions. Use patterns like: improving quality, protecting elements, improving the ability of something to support or enable something else. Assume the model covers the important things, so focus on how those things can be better in the real world. Provide the output as a Markdown list."
Console.Log("Generating questions...")
response = ai.prompt(text=prompt)

# 4. Append to Document
markdown.append_text(doc=doc_id, text="## Questions for System Improvement\\n" + response)
markdown.open_doc(id=doc_id)
ProgressBar.Hide()
Console.Log("Done.")`
  },
...
for n in nodes:
    ...
    idx = idx + 1
    pct = (idx / nodes.length) * 100
    ProgressBar.Update(pct, "Processing: " + n.name)
...
markdown.open_doc(id=doc_id)
ProgressBar.Hide()
Console.Log("Report generated and opened.")
`
  },
  {
    id: 'progress_bar_demo',
    name: 'Progress Bar Demo',
    description: 'Demonstrates the progress bar by iterating over all nodes.',
    code: `# Progress Bar Demo
# This script iterates over all nodes and updates the progress bar.

nodes = graph.get_all_nodes()
Console.Log("Starting demo with " + nodes.length + " nodes...")
ProgressBar.Show()

count = 0
for n in nodes:
    count = count + 1
    pct = (count / nodes.length) * 100
    
    ProgressBar.Update(pct, "Processing: " + n.name)
    Console.Log("Processed: " + n.name)
    
    # Sleep to simulate work and allow UI update
    sleep(0.3)
...
export const DEFAULT_COLOR_SCHEMES: ColorScheme[] = [
  {
    ...
    tagColors: {
      ...
      'Location': '#a8a29e', // stone-400
      'Data': '#d8b4fe', // purple-300
    },
    tagDescriptions: {
  ...
  },
  {
    ...
    relationshipDefinitions: [
      ...
      { label: 'contributes to', description: 'Provides input or assistance towards a task or goal.' }
    ],
