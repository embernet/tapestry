...
export class ScriptEngine {
    ...
    createContext(toolRegistry: ToolRegistry, callbacks: {
        ...
        // Inject APIs
        ctx.vars['Console'] = {
            Log: (msg: any) => callbacks.log(String(msg))
        };

        ctx.vars['ProgressBar'] = {
            Show: () => callbacks.progressBar.show(),
            Update: (p: number, m?: string) => callbacks.progressBar.update(p, m),
            Hide: () => callbacks.progressBar.hide()
        };
                    ...
                    if (op.toolId && op.action) {
                        ...
                        // 2. Try Variable Method Call
                            ...
                            if (variable !== undefined) {
                                ...
                                // Generic Object Method Call (for injected APIs like ProgressBar)
                                else if (typeof variable === 'object' && variable !== null) {
                                    if (typeof variable[op.action] === 'function') {
                                        // Map positional args to array for function call if needed, 
                                        // or strict mapping. Our simplified engine uses named/positional mixed.
                                        // For ProgressBar.Update(percentage, message), we expect args '0', '1' or named.

                                        // Simple strategy: check function signature or just pass values?
                                        // Since we defined the API wrappers manually in createContext, we can control this.
                                        // Let's pass the raw args object or resolved values? 
                                        // The Previous logic resolved args. 

                                        // For Console.Log(msg), arg is '0'.
                                        // For ProgressBar.Update(p, m), args are '0', '1'.

                                        const resolvedArgs = this.resolveArgs(op.args || {}, ctx);
                                        const argValues = Object.keys(resolvedArgs).sort().map(k => resolvedArgs[k]);

                                        // Handle named args specific to our API if needed, but for now expect positional usage in scripts mostly,
                                        // or handle standard mapping.

                                        // Actually, let's just pass the resolved args to a flexible wrapper or 
                                        // assume the wrapper handles it? 
                                        // Best approach for TScript's mixed bag:
                                        // We injected: Update: (p, m) => ...
                                        // Script calls: ProgressBar.Update(50, "msg") which becomes args={'0':50, '1':"msg"}

                                        // We need to convert that dict back to ordered args for the JS function apply.
                                        const argList = [];
                                        let i = 0;
                                        while (resolvedArgs[String(i)] !== undefined) {
                                            argList.push(resolvedArgs[String(i)]);
                                            i++;
                                        }

                                        result = variable[op.action].apply(variable, argList);
                                        executed = true;
                                    }
                                }
                            }
