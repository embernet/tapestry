...
const AppHeader: React.FC<AppHeaderProps> = ({
    ...
    return (
        <div className="absolute top-4 left-4 z-[600] flex flex-col items-start gap-2">
            <div className={`p-2 rounded-lg flex items-center space-x-2 ${headerBg}`}>
                ...
                <div className="relative" ref={mainMenuRef}>
                    ...
                    {isMainMenuOpen && (
                        <div className={`absolute top-full left-0 mt-2 w-64 border rounded-lg shadow-xl py-2 flex flex-col z-[950] text-sm animate-fade-in-down max-h-[80vh] overflow-y-auto ${menuBg}`}>
                            {/* File Section */}
                    ...
                    {isSystemMenuOpen && (
                        <div className={`absolute top-full left-0 mt-2 w-64 border rounded-lg shadow-xl py-2 flex flex-col z-[950] text-sm animate-fade-in-down ${menuBg}`}>
                ...
                <button onClick={onToggleTheme} title={isDarkMode ? "Light Mode" : "Dark Mode"} className={`p-2 rounded-md transition ${textClass}`}>
                    {isDarkMode ? (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                        </svg>
                    ) : (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
