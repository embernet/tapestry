...
export default function App() {
    ...
    console.log('[DEBUG] App Render. Elements count:', elements.length);
    ...
    const {
        ...
        handleDeleteView, handleRenameView, handleUpdateActiveView, handleHideFromView, handleUnhideFromView, handleClearExclusions, handleGenerateTapestry,
    ...
    const tools = useTools(panelState, {
        onOpenSettings: (tab) => {
            setSettingsInitialTab((tab || 'general') as any);
    ...
    const handleNodeMove = useCallback((id: string, x: number, y: number) => {
        ...
        const isDefaultView = !activeView || (views.length > 0 && activeViewId === views[0].id) || activeView.name === 'Full Graph';
    ...
    useScriptTools({
        ...
        aiConfig,
        setChatConversation,
        setIsChatPanelOpen: panelState.setIsChatPanelOpen,
    ...
    useEffect(() => {
        if (persistence.currentModelId) {
            document.title = `TS: ${persistence.currentModelName}`;
        } else {
            document.title = "Tapestry Studio";
        }
    }, [persistence.currentModelId, persistence.currentModelName]);
    ...
    const handleOpenDocument = useCallback((docId: string, origin?: string) => {
        setOpenDocIds(prev => { if (prev.includes(docId)) return prev; return [...prev, docId]; });

        // Open as floating panel by default
        const width = 600;
        const height = 800;
        const offset = Math.floor(Math.random() * 40) - 20; // Slight random offset
        const x = Math.max(100, (window.innerWidth - width) / 2 + offset);
        const y = Math.max(60, (window.innerHeight - height) / 2 + offset);

        openPanelAt(`doc-${docId}`, {
    ...
    const handleCreateDocument = useCallback((folderId: string | null, type: string = 'text', data?: any) => {
        const now = new Date().toISOString();
        const newDoc: TapestryDocument = { id: generateUUID(), title: 'Untitled Document', content: '', folderId, createdAt: now, updatedAt: now, type, data };
    ...
    const handleAddToKanbanWrapper = useCallback((ids: string[], coords: { x: number; y: number }, boardId?: string) => {
        ...
        // Fix: If no board exists at all (and no target specified), create a default one automatically
        // instead of falling back to generic "Status" which causes inconsistency.
        if (!targetBoard) {
            const newBoard = createKanbanBoard("Kanban Board");
        ...
        const defaultColumn = targetBoard.columns[0];
    ...
    return (
        <div className="w-screen h-screen overflow-hidden flex flex-col relative">
            ...
            {persistence.isQuotaExceeded && (
                <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-[5000] bg-red-900/95 text-white px-6 py-4 rounded-lg shadow-2xl flex items-center gap-4 animate-fade-in-down border border-red-500 backdrop-blur-md max-w-2xl">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-red-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <div className="flex-grow">
                        <h3 className="font-bold text-lg text-red-100">Browser local storage quota exceeded</h3>
                        <p className="text-sm text-red-200">
                            Tapestry can no longer autosave your changes to browser storage.
                            <strong className="block mt-1 text-white">Please save your model by pressing the Save button on the toolbar.</strong>
                            To fix this, clear space by deleting old models from the "Recovered Models" menu.
                        </p>
                    </div>
                    <button
                        onClick={() => persistence.setIsQuotaExceeded(false)}
            ...
            {persistence.currentModelId && !isPresenting && (
                <AppHeader
                    currentModelName={persistence.currentModelId ? persistence.currentModelName : ''}
                    ...
                    {persistence.currentModelId ? (
                        <>
                            ...
                            <ContextMenus contextMenu={contextMenu} relationshipContextMenu={relationshipContextMenu} canvasContextMenu={canvasContextMenu} relationships={relationships} panelState={panelState} persistence={persistence} onCloseContextMenu={handleCloseContextMenu} onCloseRelationshipContextMenu={handleCloseRelationshipContextMenu} onCloseCanvasContextMenu={handleCloseCanvasContextMenu} onDeleteElement={handleDeleteElement} onAddRelationshipFromContext={(id) => { setPanelStateUI({ view: 'addRelationship', sourceElementId: id, targetElementId: null, isNewTarget: false }); setSelectedElementId(null); setMultiSelection(new Set()); setSelectedRelationshipId(null); }} onDeleteRelationship={handleDeleteRelationship} onChangeRelationshipDirection={handleChangeRelationshipDirection} onZoomToFit={handleZoomToFit} onAutoLayout={handleStaticLayout} onSaveAsImage={handleSaveAsImage} importFileRef={importFileRef} isDarkMode={isDarkMode} onToggleNodeHighlight={handleToggleNodeHighlight} elements={elements} onHideFromView={handleHideFromView} multiSelection={multiSelection} onAddToKanban={handleAddToKanbanWrapper} onRemoveFromKanban={handleRemoveFromKanbanWrapper} onMoveToBoard={handleMoveToBoardWrapper} onCreateBoardAndMove={handleCreateBoardAndMoveWrapper} onCreateBoardAndAdd={handleCreateBoardAndAddWrapper} onMoveToColumn={handleMoveToColumnWrapper} kanbanBoards={persistence.kanbanBoards} activeKanbanBoardId={activeKanbanBoardId}
                                onAddLink={(id) => {
                                    ...
                                    if (url !== null) {
                                        ...
                                        if (url) newAttributes.url = url;
                                        handleUpdateElement({ ...element, attributes: newAttributes });
                                    }
                                }}
                                onEditLink={(id) => {
                                    const element = elements.find(e => e.id === id);
                                    ...
                                    if (element.attributes) {
                                        const key = Object.keys(element.attributes).find(k => k.toLowerCase() === 'url');
                                        if (key) currentUrl = element.attributes[key];
                                    }
                                    const url = window.prompt("Edit URL:", currentUrl);
                                    if (url !== null) {
                                        const newAttributes = { ...(element.attributes || {}) };
                                        // Case-insensitive removal
                                        Object.keys(newAttributes).forEach(k => { if (k.toLowerCase() === 'url') delete newAttributes[k]; });
                                        if (url) newAttributes.url = url;
                                        handleUpdateElement({ ...element, attributes: newAttributes });
                                    }
                                }}
                                onOpenLink={(id) => {
                                    ...
                                    const key = Object.keys(element.attributes).find(k => k.toLowerCase() === 'url');
                                    if (key && element.attributes[key]) {
                                        window.open(element.attributes[key], '_blank');
                                    }
                                }}
                            />
                            ...
                            <ScriptPanel
                                ...
                                aiConfig={aiConfig}
                                allocateZIndex={allocateZIndex}
                                activeScriptId={panelState.activeScriptId}
                                setActiveScriptId={panelState.setActiveScriptId}
                                mode={panelState.scriptPanelMode}
                                setMode={panelState.setScriptPanelMode}
                            />
