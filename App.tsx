...
export default function App() {
  ...
  // ... (Tool functions same as original) ...
  const getToolPrompt = useCallback((tool: string, subTool?: string | null) => {
      ...
      if (subTool && prompts[`${tool}:${subTool}`]) return prompts[`${tool}:${subTool}`];
      if (prompts[tool]) return prompts[tool];
      return DEFAULT_TOOL_PROMPTS[tool];
  }, [systemPromptConfig]);

  const toggleTool = (toolName: string) => {
      setActiveTool(prev => {
          if (prev === 'bulk' && toolName !== 'bulk') setIsBulkEditActive(false);
  ...
  const handleLogHistory = useCallback((tool: string, content: string, summary?: string, subTool?: string, toolParams?: any) => {
      ...
      const newEntry: HistoryEntry = { id: generateUUID(), tool, subTool, toolParams, timestamp: now, content, summary };
  ...
  const handleDetachHistory = useCallback((id: string) => {
      if (!detachedHistoryIds.includes(id)) {
          ...
          setPanelLayouts(prev => ({
              ...prev,
              [`history-${id}`]: { x: window.innerWidth / 2 - 250, y: window.innerHeight / 2 - 200, w: 500, h: 400, zIndex: nextZ, isFloating: true }
          }));
      }
  }, [detachedHistoryIds]);

  // ... (Tool Opening Logic, Ref Handling, State same as original) ...
  // (Skipping duplicate lines for brevity, assuming unchanged logic is preserved in final output)
  
  const handleOpenTool = useCallback((tool: string, subTool?: string) => {
      setIsToolsPanelOpen(true);
      setActiveTool(tool);
      if (tool === 'triz') { setActiveTrizTool((subTool as TrizToolType) || 'contradiction'); setIsTrizModalOpen(true); } 
      else if (tool === 'lss') { setActiveLssTool((subTool as LssToolType) || 'dmaic'); setIsLssModalOpen(true); } 
      else if (tool === 'toc') { setActiveTocTool((subTool as TocToolType) || 'crt'); setIsTocModalOpen(true); } 
      else if (tool === 'ssm') { setActiveSsmTool((subTool as SsmToolType) || 'rich_picture'); setIsSsmModalOpen(true); } 
      else if (tool === 'scamper') { setScamperInitialDoc(null); setIsScamperModalOpen(true); } 
      else if (tool === 'explorer') { handleExplorerToolSelect((subTool as ExplorerToolType) || 'treemap'); }
      else if (tool === 'tagcloud') { handleTagCloudToolSelect((subTool as TagCloudToolType) || 'tags'); }
      else if (tool === 'swot') { setActiveSwotTool((subTool as SwotToolType) || 'matrix'); setSwotInitialDoc(null); setIsSwotModalOpen(true); } 
      else if (tool === 'mermaid') { setIsMermaidPanelOpen(true); }
  }, []);

  const handleReopenHistory = useCallback((entry: HistoryEntry) => {
      // Same logic as original, just ensuring tool open calls match
      const toolId = entry.tool.split(':')[0].toLowerCase().trim(); 
      const subTool = entry.subTool;
      const params = entry.toolParams;
      if (toolId.includes('triz')) { setActiveTool('triz'); setActiveTrizTool((subTool as TrizToolType) || null); setTrizInitialParams(params); setIsTrizModalOpen(true); } 
      else if (toolId.includes('lss')) { setActiveTool('lss'); setActiveLssTool((subTool as LssToolType) || null); setLssInitialParams(params); setIsLssModalOpen(true); } 
      else if (toolId.includes('toc')) { setActiveTool('toc'); setActiveTocTool((subTool as TocToolType) || null); setTocInitialParams(params); setIsTocModalOpen(true); } 
      else if (toolId.includes('ssm')) { setActiveTool('ssm'); setActiveSsmTool((subTool as SsmToolType) || null); setSsmInitialParams(params); setIsSsmModalOpen(true); } 
      else if (toolId.includes('scamper')) { setActiveTool('scamper'); if (params) setScamperTrigger({ operator: params.operator, letter: params.letter }); setIsScamperModalOpen(true); } 
      else if (toolId.includes('swot')) { setActiveTool('swot'); setActiveSwotTool((subTool as SwotToolType) || 'matrix'); setSwotInitialDoc(null); setIsSwotModalOpen(true); }
      setIsToolsPanelOpen(true);
  }, []);

  // Tool Selectors
  const handleTrizToolSelect = (tool: TrizToolType) => { setActiveTrizTool(tool); setIsTrizModalOpen(true); setTrizInitialParams(null); };
  const handleLssToolSelect = (tool: LssToolType) => { setActiveLssTool(tool); setIsLssModalOpen(true); setLssInitialParams(null); };
  const handleTocToolSelect = (tool: TocToolType) => { setActiveTocTool(tool); setIsTocModalOpen(true); setTocInitialParams(null); };
  const handleSsmToolSelect = (tool: SsmToolType) => { setActiveSsmTool(tool); setIsSsmModalOpen(true); setSsmInitialParams(null); };
  const handleExplorerToolSelect = (tool: ExplorerToolType) => {
      if (tool === 'treemap') setIsTreemapPanelOpen(p => !p);
      if (tool === 'tags') setIsTagDistPanelOpen(p => !p);
      if (tool === 'relationships') setIsRelDistPanelOpen(p => !p);
      if (tool === 'sunburst') { setIsSunburstPanelOpen(p => !p); setSunburstState(p => ({ ...p, active: true })); }
      setActiveTool(null);
  };
  const handleTagCloudToolSelect = (tool: TagCloudToolType) => {
      if (tool === 'tags') setIsConceptCloudOpen(p => !p);
      if (tool === 'nodes') setIsInfluenceCloudOpen(p => !p);
      if (tool === 'words') setIsTextAnalysisOpen(p => !p);
      if (tool === 'full_text') setIsFullTextAnalysisOpen(p => !p);
  };
  const handleSwotToolSelect = (tool: SwotToolType) => { setActiveSwotTool(tool); setSwotInitialDoc(null); setIsSwotModalOpen(true); setActiveTool(null); };
  const handleMermaidToolSelect = (tool: MermaidToolType) => { if (tool === 'editor') setIsMermaidPanelOpen(true); };

  useEffect(() => { if (activeTool !== 'bulk') setIsBulkEditActive(false); }, [activeTool]);
  ...
  // Migration Logic
  const migrateLegacySchemes = useCallback((loadedSchemes: ColorScheme[]) => {
    ...
    const migratedSchemes = loadedSchemes.map(s => {
        ...
        if (s.relationshipLabels && !s.relationshipDefinitions) {
            if (defaultScheme && defaultScheme.relationshipDefinitions) {
                ...
                s.relationshipDefinitions = [ ...defaultScheme.relationshipDefinitions, ...extraLabels.map(l => ({ label: l, description: '' })) ];
            } else { s.relationshipDefinitions = s.relationshipLabels.map(l => ({ label: l, description: '' })); }
        ...
        if (defaultScheme) {
            ...
            if (missingTags.length > 0) {
                ...
                s.tagDescriptions = { ...defaultScheme.tagDescriptions, ...s.tagDescriptions };
                changes.push(`Updated schema '${s.name}': Added missing tags.`);
            }
            if (s.relationshipDefinitions && defaultScheme.relationshipDefinitions) {
                ...
                const missingDefs = defaultScheme.relationshipDefinitions.filter(d => !currentLabels.includes(d.label));
                if (missingDefs.length > 0) { s.relationshipDefinitions = [...s.relationshipDefinitions, ...missingDefs]; changes.push(`Updated schema '${s.name}': Added missing relations.`); }
            }
        }
        return s;
    });
  ...
  const loadModelData = useCallback((data: any, modelId: string, modelMetadata?: ModelMetadata) => {
    ...
    setOpenDocIds([]); setDetachedHistoryIds([]); setPanelLayouts({});
    setAnalysisHighlights(new Map()); setAnalysisFilterState({ mode: 'none', ids: new Set() }); setMultiSelection(new Set()); setSelectedElementId(null);
    ...
    if (missingDefaults.length > 0) { finalSchemes = [...migratedSchemes, ...missingDefaults]; changes.push(`Added ${missingDefaults.length} new standard schemas.`); }
    setColorSchemes(finalSchemes);
    setActiveSchemeId(data.activeSchemeId || DEFAULT_COLOR_SCHEMES[0]?.id || null);
    if (changes.length > 0) { setSchemaUpdateChanges(changes); setIsSchemaUpdateModalOpen(true); }
    if (data.systemPromptConfig) { setSystemPromptConfig({ ...DEFAULT_SYSTEM_PROMPT_CONFIG, ...data.systemPromptConfig }); } else { setSystemPromptConfig(DEFAULT_SYSTEM_PROMPT_CONFIG); }
    ...
    if (modelMetadata && !modelMetadata.filename) currentFileHandleRef.current = null;
    if (modelMetadata) { setModelsIndex(prevIndex => { const exists = prevIndex.find(m => m.id === modelId); if (exists) return prevIndex.map(m => m.id === modelId ? { ...m, ...modelMetadata } : m); return [...prevIndex, modelMetadata]; }); }
  }, [migrateLegacySchemes]);

  // ... (Handle Load Model, Create Model, Persistence Effects - Same as original) ...
  ...
  const handleGenerateMermaid = useCallback(async (prompt: string, contextMarkdown?: string) => {
      ...
      try {
          ...
          // Use aiService instead of direct GoogleGenAI
  ...
  // ... (All other action handlers: AddElement, DeleteElement, etc. - Same as original) ...
  ...
  const handleOpenDocument = useCallback((docId: string, origin?: 'report') => {
        ...
        if (!doc) return;
        if (doc.type === 'swot-analysis') { setActiveTool('swot'); setActiveSwotTool('matrix'); setSwotInitialDoc(doc); setIsSwotModalOpen(true); return; } 
        else if (doc.type === 'scamper-analysis') { setActiveTool('scamper'); setScamperInitialDoc(doc); setIsScamperModalOpen(true); return; }
  ...
  const aiActions: ModelActions = useMemo(() => {
    const findElementByName = (name: string): Element | undefined => { return elementsRef.current.find(e => e.name.toLowerCase() === name.toLowerCase()); };
    const findDocumentByTitle = (title: string): TapestryDocument | undefined => { return documentsRef.current.find(d => d.title.toLowerCase() === title.toLowerCase()); };
    return {
      addElement: (data) => { const now = new Date().toISOString(); const id = generateUUID(); const count = elementsRef.current.length; const angle = count * 0.5; const radius = 50 + (5 * count); const centerX = window.innerWidth / 2; const centerY = window.innerHeight / 2; const x = centerX + radius * Math.cos(angle); const y = centerY + radius * Math.sin(angle); const newElement: Element = { id, name: data.name, notes: data.notes || '', tags: data.tags || [], attributes: data.attributes || {}, createdAt: now, updatedAt: now, x, y, fx: x, fy: y }; elementsRef.current = [...elementsRef.current, newElement]; setElements(prev => [...prev, newElement]); return id; },
      updateElement: (name, data) => { const element = findElementByName(name); if (!element) return false; const updatedElement = { ...element, ...data, updatedAt: new Date().toISOString() }; if (data.tags) { updatedElement.tags = Array.from(new Set([...element.tags, ...data.tags])); } elementsRef.current = elementsRef.current.map(e => e.id === element.id ? updatedElement : e); setElements(prev => prev.map(e => e.id === element.id ? updatedElement : e)); return true; },
      deleteElement: (name) => { const element = findElementByName(name); if (!element) return false; elementsRef.current = elementsRef.current.filter(f => f.id !== element.id); relationshipsRef.current = relationshipsRef.current.filter(r => r.source !== element.id && r.target !== element.id); handleDeleteElement(element.id); return true; },
      addRelationship: (sourceName, targetName, label, directionStr) => { const source = findElementByName(sourceName); const target = findElementByName(targetName); if (!source || !target) return false; let direction = RelationshipDirection.To; if (directionStr) { if (directionStr.toUpperCase() === 'FROM') direction = RelationshipDirection.From; if (directionStr.toUpperCase() === 'NONE') direction = RelationshipDirection.None; } const newRel: Relationship = { id: generateUUID(), source: source.id, target: target.id, label: label, direction: direction, tags: [] }; relationshipsRef.current = [...relationshipsRef.current, newRel]; setRelationships(prev => [...prev, newRel]); return true; },
      deleteRelationship: (sourceName, targetName) => { const source = findElementByName(sourceName); const target = findElementByName(targetName); if (!source || !target) return false; relationshipsRef.current = relationshipsRef.current.filter(r => { const isMatch = (r.source === source.id && r.target === target.id) || (r.source === target.id && r.target === source.id); return !isMatch; }); setRelationships(prev => prev.filter(r => { const isMatch = (r.source === source.id && r.target === target.id) || (r.source === target.id && r.target === source.id); return !isMatch; })); return true; },
      setElementAttribute: (elementName, key, value) => { const element = findElementByName(elementName); if (!element) return false; const attributes = { ...(element.attributes || {}), [key]: value }; const updated = { ...element, attributes, updatedAt: new Date().toISOString() }; elementsRef.current = elementsRef.current.map(e => e.id === element.id ? updated : e); setElements(prev => prev.map(e => e.id === element.id ? updated : e)); return true; },
      deleteElementAttribute: (elementName, key) => { const element = findElementByName(elementName); if (!element) return false; const attributes = { ...(element.attributes || {}) }; delete attributes[key]; const updated = { ...element, attributes, updatedAt: new Date().toISOString() }; elementsRef.current = elementsRef.current.map(e => e.id === element.id ? updated : e); setElements(prev => prev.map(e => e.id === element.id ? updated : e)); return true; },
      setRelationshipAttribute: (sourceName, targetName, key, value) => { const source = findElementByName(sourceName); const target = findElementByName(targetName); if (!source || !target) return false; let relIndex = relationshipsRef.current.findIndex(r => r.source === source.id && r.target === target.id); if (relIndex === -1) { relIndex = relationshipsRef.current.findIndex(r => r.source === target.id && r.target === source.id); } if (relIndex === -1) return false; const rel = relationshipsRef.current[relIndex]; const attributes = { ...(rel.attributes || {}), [key]: value }; const updated = { ...rel, attributes }; relationshipsRef.current = [...relationshipsRef.current]; relationshipsRef.current[relIndex] = updated; setRelationships(prev => prev.map(r => r.id === rel.id ? updated : r)); return true; },
      deleteRelationshipAttribute: (sourceName, targetName, key) => { const source = findElementByName(sourceName); const target = findElementByName(targetName); if (!source || !target) return false; let relIndex = relationshipsRef.current.findIndex(r => r.source === source.id && r.target === target.id); if (relIndex === -1) { relIndex = relationshipsRef.current.findIndex(r => r.source === target.id && r.target === source.id); } if (relIndex === -1) return false; const rel = relationshipsRef.current[relIndex]; const attributes = { ...(rel.attributes || {}) }; delete attributes[key]; const updated = { ...rel, attributes }; relationshipsRef.current = [...relationshipsRef.current]; relationshipsRef.current[relIndex] = updated; setRelationships(prev => prev.map(r => r.id === rel.id ? updated : r)); return true; },
      readDocument: (title) => { const doc = findDocumentByTitle(title); return doc ? doc.content : null; },
      createDocument: (title, content = '', type = 'text', data = null) => { const now = new Date().toISOString(); const newDoc: TapestryDocument = { id: generateUUID(), title, content, folderId: null, createdAt: now, updatedAt: now, type, data }; documentsRef.current = [...documentsRef.current, newDoc]; setDocuments(prev => [...prev, newDoc]); if (!openDocIds.includes(newDoc.id)) { setOpenDocIds(prev => [...prev, newDoc.id]); } return newDoc.id; },
      ...
      createFolder: (name, parentId) => { const id = generateUUID(); const newFolder: TapestryFolder = { id, name, parentId: parentId || null, createdAt: new Date().toISOString() }; foldersRef.current = [...foldersRef.current, newFolder]; setFolders(prev => [...prev, newFolder]); return id; },
      moveDocument: (docId, folderId) => { const doc = documentsRef.current.find(d => d.id === docId); if (!doc) return false; const updatedDoc = { ...doc, folderId, updatedAt: new Date().toISOString() }; documentsRef.current = documentsRef.current.map(d => d.id === docId ? updatedDoc : d); setDocuments(prev => prev.map(d => d.id === docId ? updatedDoc : d)); return true; }
    };
  }, [handleDeleteElement, openDocIds]);

  // ... (Disk Save, Save As, Import handlers - Same as original) ...
  const handleDiskSave = useCallback(async () => {
    ...
    try {
        if (!isInIframe() && currentFileHandleRef.current && 'createWritable' in currentFileHandleRef.current) { const writable = await currentFileHandleRef.current.createWritable(); await writable.write(jsonString); await writable.close(); } else if (!isInIframe() && 'showSaveFilePicker' in window) { const options = { suggestedName: updatedMetadata.filename, types: [{ description: 'JSON Files', accept: {'application/json': ['.json']}, }], }; const fileHandle = await (window as any).showSaveFilePicker(options); currentFileHandleRef.current = fileHandle; const writable = await fileHandle.createWritable(); await writable.write(jsonString); await writable.close(); } else { const blob = new Blob([jsonString], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = updatedMetadata.filename!; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }
        setModelsIndex(prev => prev.map(m => m.id === currentModelId ? updatedMetadata : m)); localStorage.setItem(MODELS_INDEX_KEY, JSON.stringify(modelsIndex.map(m => m.id === currentModelId ? updatedMetadata : m))); localStorage.setItem(`${MODEL_DATA_PREFIX}${currentModelId}`, JSON.stringify(modelData));
  ...
  const handleSaveAs = useCallback((name: string, description: string) => { if (!currentModelId) return; const now = new Date().toISOString(); const newId = generateUUID(); const modelData = { elements, relationships, documents, folders, colorSchemes, activeSchemeId, systemPromptConfig, history, slides, mermaidDiagrams }; const currentHash = computeContentHash(modelData); const newMetadata: ModelMetadata = { id: newId, name, description, createdAt: now, updatedAt: now, filename: `${name.replace(/ /g, '_')}.json`, contentHash: currentHash }; try { localStorage.setItem(`${MODEL_DATA_PREFIX}${newId}`, JSON.stringify(modelData)); setModelsIndex(prev => { const newIndex = [...prev, newMetadata]; localStorage.setItem(MODELS_INDEX_KEY, JSON.stringify(newIndex)); return newIndex; }); setCurrentModelId(newId); setIsSaveAsModalOpen(false); currentFileHandleRef.current = null; } catch (e) { console.error("Save As failed", e); alert("Failed to save copy. Local storage might be full."); } }, [currentModelId, elements, relationships, documents, folders, colorSchemes, activeSchemeId, systemPromptConfig, history, slides, mermaidDiagrams]);

  const processImportedData = useCallback((text: string, filename?: string) => { try { const imported = JSON.parse(text); let dataToImport: any = null; let nameToUse = 'Imported Model'; let descToUse = ''; let existingId: string | null = null; let importedHash: string = ''; if (imported.metadata && imported.data && Array.isArray(imported.data.elements)) { dataToImport = imported.data; nameToUse = imported.metadata.name || nameToUse; descToUse = imported.metadata.description || ''; existingId = imported.metadata.id; importedHash = computeContentHash(dataToImport); } else if (imported.elements && Array.isArray(imported.elements)) { dataToImport = imported; importedHash = computeContentHash(dataToImport); } if (!dataToImport) { throw new Error('Invalid file format.'); } if (!dataToImport.relationships) dataToImport.relationships = []; if (existingId) { const localDataStr = localStorage.getItem(`${MODEL_DATA_PREFIX}${existingId}`); if (localDataStr) { const localIndex = modelsIndex.find(m => m.id === existingId); if (localIndex) { const localHash = localIndex.contentHash || computeContentHash(JSON.parse(localDataStr)); if (localHash !== importedHash) { setPendingImport({ localMetadata: localIndex, diskMetadata: { ...imported.metadata, filename: filename || imported.metadata.filename, contentHash: importedHash, lastDiskHash: importedHash }, localData: JSON.parse(localDataStr), diskData: dataToImport }); return; } } } } const now = new Date().toISOString(); const newModelId = existingId || generateUUID(); if (!existingId) { let finalModelName = nameToUse; let i = 1; while(modelsIndex.some(m => m.name === finalModelName)) { i++; finalModelName = `${nameToUse} ${i}`; } nameToUse = finalModelName; } const newMetadata: ModelMetadata = { id: newModelId, name: nameToUse, description: descToUse, createdAt: imported.metadata?.createdAt || now, updatedAt: imported.metadata?.updatedAt || now, filename: filename, contentHash: importedHash, lastDiskHash: importedHash }; const newModelData = { elements: dataToImport.elements || [], relationships: dataToImport.relationships || [], documents: dataToImport.documents || [], folders: dataToImport.folders || [], history: dataToImport.history || [], slides: dataToImport.slides || [], mermaidDiagrams: dataToImport.mermaidDiagrams || [], colorSchemes: dataToImport.colorSchemes || DEFAULT_COLOR_SCHEMES, activeSchemeId: dataToImport.activeSchemeId || DEFAULT_COLOR_SCHEMES[0]?.id || null, systemPromptConfig: dataToImport.systemPromptConfig || DEFAULT_SYSTEM_PROMPT_CONFIG, }; loadModelData(newModelData, newModelId, newMetadata); } catch (error) { const message = error instanceof Error ? error.message : 'An unknown error occurred.'; alert(`Failed to import file: ${message}`); console.error("Import failed:", error); } }, [modelsIndex, loadModelData]);
  
  const handleImportClick = useCallback(async () => { if (!isInIframe() && 'showOpenFilePicker' in window) { try { const pickerOptions = { types: [{ description: 'JSON Files', accept: { 'application/json': ['.json'] } }], }; const [fileHandle] = await (window as any).showOpenFilePicker(pickerOptions); currentFileHandleRef.current = fileHandle; const file = await fileHandle.getFile(); const text = await file.text(); processImportedData(text, file.name); return; } catch (err: any) { if (err.name !== 'AbortError') { console.warn("File System Access API failed, falling back to input.", err); } else { return; } } } if (importFileRef.current) { importFileRef.current.value = ''; importFileRef.current.click(); } }, [processImportedData]);
  ...
  // ... (Other handlers: Node Click, Context Menus, Simulation, etc.) ...
  const handleCloseContextMenu = useCallback(() => setContextMenu(null), []);
  const handleCloseCanvasContextMenu = useCallback(() => setCanvasContextMenu(null), []);
  const runImpactSimulation = (startNodeId: string) => { const newSimState: Record<string, SimulationNodeState> = {}; const queue: { id: string, state: SimulationNodeState }[] = []; newSimState[startNodeId] = simulationState[startNodeId] === 'increased' ? 'decreased' : 'increased'; queue.push({ id: startNodeId, state: newSimState[startNodeId] }); const visited = new Set<string>(); visited.add(startNodeId); const isPositive = (label: string) => ['causes', 'increases', 'promotes', 'leads to', 'produces', 'enables', 'enhances', 'amplifies', 'reinforces'].some(k => label.toLowerCase().includes(k)); const isNegative = (label: string) => ['inhibits', 'decreases', 'prevents', 'reduces', 'stops', 'block', 'counteracts'].some(k => label.toLowerCase().includes(k)); while (queue.length > 0) { const { id, state } = queue.shift()!; const outgoing = relationships.filter(r => r.source === id); outgoing.forEach(rel => { const targetId = rel.target as string; if (visited.has(targetId)) return; let nextState: SimulationNodeState = 'neutral'; if (isPositive(rel.label)) { nextState = state; } else if (isNegative(rel.label)) { nextState = state === 'increased' ? 'decreased' : 'increased'; } if (nextState !== 'neutral') { newSimState[targetId] = nextState; visited.add(targetId); queue.push({ id: targetId, state: nextState }); } }); } setSimulationState(newSimState); };
  ...
  const handleNodeClick = useCallback((elementId: string, event: MouseEvent) => { 
      ...
      if (isSunburstPanelOpen && sunburstState.active) { if (!originalElements && !sunburstState.centerId) { setOriginalElements(elements); } const cx = window.innerWidth / 2; const cy = window.innerHeight / 2; setElements(prev => prev.map(e => { if (e.id === elementId) { return { ...e, x: cx, y: cy, fx: cx, fy: cy, vx: 0, vy: 0 }; } return { ...e, fx: null, fy: null }; })); setSunburstState(prev => ({ ...prev, centerId: elementId })); setIsPhysicsModeActive(true); setSelectedElementId(elementId); setMultiSelection(new Set([elementId])); setTimeout(() => { if (graphCanvasRef.current) { graphCanvasRef.current.setCamera(0, 0, 1); } }, 50); return; }
  ...
  // Copy/Paste
  const handleCopy = async () => { const idsToCopy = multiSelection.size > 0 ? Array.from(multiSelection) : (selectedElementId ? [selectedElementId] : []); if (idsToCopy.length === 0) return; const selectedEls = elements.filter(e => idsToCopy.includes(e.id)); const selectedRels = relationships.filter(r => idsToCopy.includes(r.source as string) && idsToCopy.includes(r.target as string)); const textReport = generateSelectionReport(selectedEls, selectedRels); setInternalClipboard({ elements: selectedEls, relationships: selectedRels }); try { await navigator.clipboard.writeText(textReport); alert(`Copied ${selectedEls.length} items to clipboard (Text Report). App-data ready for paste.`); } catch (err) { console.warn("Copy failed", err); alert("Failed to copy text to system clipboard."); } };
  const handlePaste = () => { if (!internalClipboard) { alert("Internal clipboard is empty."); return; } const { elements: pastedElements, relationships: pastedRelationships } = internalClipboard; if (pastedElements.length === 0) return; const idMap = new Map<string, string>(); const now = new Date().toISOString(); pastedElements.forEach((el: Element) => { idMap.set(el.id, generateUUID()); }); const newElements = pastedElements.map((el: Element) => ({ ...el, id: idMap.get(el.id)!, x: (el.x || 0) + 50, y: (el.y || 0) + 50, fx: (el.fx ? el.fx + 50 : null), fy: (el.fy ? el.fy + 50 : null), createdAt: now, updatedAt: now })); const newRelationships = pastedRelationships.map((rel: Relationship) => ({ ...rel, id: generateUUID(), source: idMap.get(rel.source as string) || rel.source, target: idMap.get(rel.target as string) || rel.target })); setElements(prev => [...prev, ...newElements]); setRelationships(prev => [...prev, ...newRelationships]); const newSelection = new Set(newElements.map((e: Element) => e.id)); setMultiSelection(newSelection); if (newElements.length > 0) setSelectedElementId(newElements[0].id); };

  // Markdown and Layout Handlers
  const handleApplyMarkdown = (markdown: string, shouldMerge: boolean = false) => { let processedMarkdown = markdown; processedMarkdown = processedMarkdown.replace(/\s*\/>\s*/g, ' -[Counteracts]-> '); processedMarkdown = processedMarkdown.replace(/(?<!\-|\[)>(?!\-|\])/g, ' -[Produces]-> '); const lines = processedMarkdown.split('\n').filter(line => { const trimmed = line.trim(); return trimmed !== '' && !trimmed.startsWith('#'); }); const parsedElements = new Map<string, { tags: string[] }>(); const parsedRels: { sourceName: string, targetName: string, label: string, direction: RelationshipDirection }[] = []; function parseElementStr(str: string) { let workStr = str.trim(); if (!workStr) return null; let name: string; let tags: string[] = []; const lastColonIndex = workStr.lastIndexOf(':'); const lastParenOpenIndex = workStr.lastIndexOf('('); if (lastColonIndex > -1 && lastColonIndex > lastParenOpenIndex) { const tagsStr = workStr.substring(lastColonIndex + 1); tags = tagsStr.split(',').map(t => t.trim()).filter(t => !!t); workStr = workStr.substring(0, lastColonIndex).trim(); } if (workStr.endsWith('+')) { workStr = workStr.slice(0, -1).trim(); tags.push('Useful'); } else if (workStr.endsWith('-')) { workStr = workStr.slice(0, -1).trim(); tags.push('Harmful'); } name = workStr; if (name.startsWith('"') && name.endsWith('"')) { name = name.substring(1, name.length - 1); } if (!name) return null; return { name, tags }; } function updateParsedElement(elementData: { name: string, tags: string[] }) { const existing = parsedElements.get(elementData.name); if (existing) { const newTags = [...new Set([...existing.tags, ...elementData.tags])]; parsedElements.set(elementData.name, { tags: newTags }); } else { parsedElements.set(elementData.name, { tags: elementData.tags }); } } for (const line of lines) { const relSeparatorRegex = /(<?-\[.*?]->?)/g; const parts = line.split(relSeparatorRegex); const tokens = parts.map(p => p.trim()).filter(t => !!t); if (tokens.length === 0) continue; if (tokens.length === 1) { const element = parseElementStr(tokens[0]); if (element) { updateParsedElement(element); } continue; } let currentSourceElementStr = tokens.shift(); while (tokens.length > 0) { const relStr = tokens.shift(); const targetsStr = tokens.shift(); if (!currentSourceElementStr || !relStr || !targetsStr) break; const sourceElementData = parseElementStr(currentSourceElementStr); if (!sourceElementData) break; updateParsedElement(sourceElementData); const singleRelRegex = /<?-\[(.*?)]->?/; const relMatch = relStr.match(singleRelRegex); if (!relMatch) break; const label = relMatch[1]; let direction = RelationshipDirection.None; if (relStr.startsWith('<-')) direction = RelationshipDirection.From; else if (relStr.endsWith('->')) direction = RelationshipDirection.To; const targetElementStrs = targetsStr.split(';').map(t => t.trim()).filter(t => !!t); for (const targetElementStr of targetElementStrs) { const targetElementData = parseElementStr(targetElementStr); if (targetElementData) { updateParsedElement(targetElementData); parsedRels.push({ sourceName: sourceElementData.name, targetName: targetElementData.name, label, direction }); } } if (targetElementStrs.length === 1) { currentSourceElementStr = targetElementStrs[0]; } else { break; } } } let nextElements: Element[] = []; let nextRelationships: Relationship[] = []; const newElementNames = new Set<string>(); if (shouldMerge) { nextElements = [...elements]; nextRelationships = [...relationships]; const existingMap = new Map<string, Element>(); const nameToIdMap = new Map<string, string>(); nextElements.forEach(e => { existingMap.set(e.name.toLowerCase(), e); nameToIdMap.set(e.name.toLowerCase(), e.id); }); parsedElements.forEach(({ tags }, name) => { const lowerName = name.toLowerCase(); const existing = existingMap.get(lowerName); if (existing) { const mergedTags = Array.from(new Set([...existing.tags, ...tags])); if (mergedTags.length !== existing.tags.length || !mergedTags.every(t => existing.tags.includes(t))) { const updated = { ...existing, tags: mergedTags, updatedAt: new Date().toISOString() }; const idx = nextElements.findIndex(e => e.id === existing.id); if (idx !== -1) nextElements[idx] = updated; existingMap.set(lowerName, updated); } } else { const now = new Date().toISOString(); const newId = generateUUID(); const newEl: Element = { id: newId, name, tags, notes: '', createdAt: now, updatedAt: now }; nextElements.push(newEl); existingMap.set(lowerName, newEl); nameToIdMap.set(lowerName, newId); newElementNames.add(name); } }); parsedRels.forEach(rel => { const sId = nameToIdMap.get(rel.sourceName.toLowerCase()); const tId = nameToIdMap.get(rel.targetName.toLowerCase()); if (sId && tId) { const exists = nextRelationships.some(r => r.source === sId && r.target === tId && r.label === rel.label && r.direction === rel.direction); if (!exists) { nextRelationships.push({ id: generateUUID(), source: sId, target: tId, label: rel.label, direction: rel.direction, tags: [] }); } } }); } else { const nameToIdMap = new Map<string, string>(); parsedElements.forEach(({ tags }, name) => { const existing = elements.find(e => e.name.toLowerCase() === name.toLowerCase()); if (existing) { const updated = { ...existing, tags, updatedAt: new Date().toISOString() }; nextElements.push(updated); nameToIdMap.set(name.toLowerCase(), existing.id); } else { const now = new Date().toISOString(); const newId = generateUUID(); const newEl: Element = { id: newId, name, tags, notes: '', createdAt: now, updatedAt: now }; nextElements.push(newEl); nameToIdMap.set(name.toLowerCase(), newId); newElementNames.add(name); } }); parsedRels.forEach(rel => { const sId = nameToIdMap.get(rel.sourceName.toLowerCase()); const tId = nameToIdMap.get(rel.targetName.toLowerCase()); if (sId && tId) { nextRelationships.push({ id: generateUUID(), source: sId, target: tId, label: rel.label, direction: rel.direction, tags: [] }); } }); } let placedNewElementsCount = 0; const positionNewElements = () => { nextElements.forEach(element => { if (newElementNames.has(element.name) && element.x === undefined) { let connectedAnchor: Element | undefined; for (const rel of nextRelationships) { let anchorId: string | undefined; if (rel.source === element.id) anchorId = rel.target as string; else if (rel.target === element.id) anchorId = rel.source as string; if (anchorId) { const potentialAnchor = nextElements.find(f => f.id === anchorId && f.x !== undefined); if (potentialAnchor) { connectedAnchor = potentialAnchor; break; } } } if (connectedAnchor && connectedAnchor.x && connectedAnchor.y) { element.x = connectedAnchor.x + (Math.random() - 0.5) * 300; element.y = connectedAnchor.y + (Math.random() - 0.5) * 300; } else { element.x = 200 + (placedNewElementsCount * 50); element.y = 200 + (placedNewElementsCount * 50); placedNewElementsCount++; } element.fx = element.x; element.fy = element.y; } }); }; positionNewElements(); positionNewElements(); setElements(nextElements); setRelationships(nextRelationships); if (!shouldMerge) setIsMarkdownPanelOpen(false); };
  ...
  const handleOpenCommandHistory = useCallback(() => { const cmdFolder = foldersRef.current.find(f => f.name === "CMD" && !f.parentId); let folderId = cmdFolder?.id; if (!folderId) { folderId = generateUUID(); const newFolder: TapestryFolder = { id: folderId, name: "CMD", parentId: null, createdAt: new Date().toISOString() }; setFolders(prev => [...prev, newFolder]); foldersRef.current.push(newFolder); } let historyDoc = documentsRef.current.find(d => d.folderId === folderId && d.title === "History"); let docId = historyDoc?.id; if (!historyDoc) { docId = generateUUID(); const newDoc: TapestryDocument = { id: docId, title: "History", content: "# Command History\n", folderId: folderId, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }; setDocuments(prev => [...prev, newDoc]); documentsRef.current.push(newDoc); } if (docId) { handleOpenDocument(docId, 'report'); } }, [handleOpenDocument]);
  ...
  const panelDefinitions: PanelDefinition[] = useMemo(() => {
    const staticPanels: PanelDefinition[] = [
        ...
        { 
            ...
            content: <MermaidPanel savedDiagrams={mermaidDiagrams} onSaveDiagram={handleSaveMermaidDiagram} onDeleteDiagram={handleDeleteMermaidDiagram} onGenerate={handleGenerateMermaid} onClose={() => setIsMermaidPanelOpen(false)} isGenerating={isMermaidGenerating} elements={elements} relationships={relationships} multiSelection={multiSelection} />, 
        ...
        { 
            ...
            content: <SunburstPanel centerNodeName={elements.find(e => e.id === sunburstState.centerId)?.name || null} hops={sunburstState.hops} visibleCount={filteredElements.length} onHopsChange={(newHops) => setSunburstState(prev => ({ ...prev, hops: newHops }))} onRestart={() => { setSunburstState(prev => ({ ...prev, centerId: null, hops: 0 })); setIsPhysicsModeActive(false); if (originalElements) { setElements(originalElements); setOriginalElements(null); } }} onReset={() => { setSunburstState(prev => ({ ...prev, hops: 0 })); graphCanvasRef.current?.setCamera(0,0,1); }} onClose={() => { setIsSunburstPanelOpen(false); setSunburstState(prev => ({ ...prev, active: false })); setIsPhysicsModeActive(false); if (originalElements) { setElements(originalElements); setOriginalElements(null); } }} />,
            isOpen: isSunburstPanelOpen,
            onToggle: () => { const willOpen = !isSunburstPanelOpen; setIsSunburstPanelOpen(willOpen); if (willOpen) { setSunburstState(prev => ({ ...prev, active: true })); } else { setSunburstState(prev => ({ ...prev, active: false })); setIsPhysicsModeActive(false); if (originalElements) { setElements(originalElements); setOriginalElements(null); } } }
        },
        { id: 'tag-dist', title: 'Tag Distribution', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>, content: <TagDistributionPanel elements={filteredElements} />, isOpen: isTagDistPanelOpen, onToggle: () => setIsTagDistPanelOpen(p => !p) },
        { id: 'rel-dist', title: 'Relationship Types', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>, content: <RelationshipDistributionPanel relationships={filteredRelationships} />, isOpen: isRelDistPanelOpen, onToggle: () => setIsRelDistPanelOpen(p => !p) },
        { id: 'concept-cloud', title: 'Tag Cloud', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>, content: <TagCloudPanel mode='tags' elements={filteredElements} relationships={filteredRelationships} onNodeSelect={(id) => handleNodeClick(id, new MouseEvent('click'))} />, isOpen: isConceptCloudOpen, onToggle: () => setIsConceptCloudOpen(p => !p) },
        { id: 'influence-cloud', title: 'Relationship Cloud', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>, content: <TagCloudPanel mode='nodes' elements={filteredElements} relationships={filteredRelationships} onNodeSelect={(id) => handleNodeClick(id, new MouseEvent('click'))} />, isOpen: isInfluenceCloudOpen, onToggle: () => setIsInfluenceCloudOpen(p => !p) },
        { id: 'text-cloud', title: 'Node Name Analysis', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>, content: <TagCloudPanel mode='words' elements={filteredElements} relationships={filteredRelationships} onNodeSelect={(id) => handleNodeClick(id, new MouseEvent('click'))} />, isOpen: isTextAnalysisOpen, onToggle: () => setIsTextAnalysisOpen(p => !p) },
        { id: 'full-text-cloud', title: 'Full Text Analysis', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg>, content: <TagCloudPanel mode='full_text' elements={filteredElements} relationships={filteredRelationships} onNodeSelect={(id) => handleNodeClick(id, new MouseEvent('click'))} />, isOpen: isFullTextAnalysisOpen, onToggle: () => setIsFullTextAnalysisOpen(p => !p) },
    ];
  ...
  return (
    <div className="w-screen h-screen overflow-hidden flex relative">
      ...
      {/* ... (Presenting View and Menus - No changes needed here, but ensuring structure is intact) ... */}
      {isPresenting && (
      ...
      {currentModelId && !isPresenting && (
          <div className="absolute top-4 left-4 z-50 bg-gray-800 bg-opacity-80 p-2 rounded-lg flex items-center space-x-2">
                ...
                <div className="relative" ref={mainMenuRef}>
                    <button onClick={() => setIsMainMenuOpen(!isMainMenuOpen)} className="flex items-center space-x-2 text-gray-300 hover:bg-gray-700 p-2 rounded-lg transition-colors outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-teal-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"><path d="M4 8c2-2 4-2 6 0s4 2 6 0" /><path d="M4 12c2-2 4-2 6 0s4 2 6 0" /><path d="M4 16c2-2 4-2 6 0s4 2 6 0" /></svg>
                        <span className="text-xl font-bold">Tapestry</span>
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 text-gray-500 transition-transform ${isMainMenuOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    {isMainMenuOpen && (
                        <div className="absolute top-full left-0 mt-2 w-64 bg-gray-900 border border-gray-600 rounded-lg shadow-xl py-2 flex flex-col z-50 text-sm animate-fade-in-down max-h-[80vh] overflow-y-auto">
                            ...
                            <button onClick={() => { handleNewModelClick(); setIsMainMenuOpen(false); }} className="w-full text-left px-4 py-2 text-gray-300 hover:bg-gray-800 hover:text-white flex items-center gap-3"><svg className="w-4 h-4 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg> New Model...</button>
                            <button onClick={() => { setIsSaveAsModalOpen(true); setIsMainMenuOpen(false); }} className="w-full text-left px-4 py-2 text-gray-300 hover:bg-gray-800 hover:text-white flex items-center gap-3"><svg className="w-4 h-4 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg> Save As...</button>
                            <button onClick={() => { handleImportClick(); setIsMainMenuOpen(false); }} className="w-full text-left px-4 py-2 text-gray-300 hover:bg-gray-800 hover:text-white flex items-center gap-3"><svg className="w-4 h-4 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg> Open Model...</button>
                            <button onClick={() => { handleDiskSave(); setIsMainMenuOpen(false); }} className="w-full text-left px-4 py-2 text-gray-300 hover:bg-gray-800 hover:text-white flex items-center gap-3"><svg className="w-4 h-4 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg> Save to Disk</button>
                            ...
                            {[ { id: 'schema', label: 'Schema', color: 'text-teal-400' }, { id: 'layout', label: 'Layout', color: 'text-orange-400' }, { id: 'analysis', label: 'Analysis', color: 'text-purple-400' }, { id: 'scamper', label: 'SCAMPER', color: 'text-cyan-400' }, { id: 'triz', label: 'TRIZ', color: 'text-indigo-400' }, { id: 'lss', label: 'Lean Six Sigma', color: 'text-blue-400' }, { id: 'toc', label: 'Theory of Constraints', color: 'text-amber-400' }, { id: 'ssm', label: 'Soft Systems', color: 'text-cyan-400' }, { id: 'swot', label: 'Strategic Analysis', color: 'text-lime-400' }, { id: 'explorer', label: 'Explorer', color: 'text-yellow-400' }, { id: 'tagcloud', label: 'Word Cloud', color: 'text-pink-400' }, { id: 'mermaid', label: 'Diagrams', color: 'text-cyan-400' }, { id: 'bulk', label: 'Bulk Edit', color: 'text-pink-400' }, { id: 'command', label: 'Command Bar', color: 'text-green-400' } ].map(tool => ( <button key={tool.id} onClick={() => { toggleTool(tool.id); setIsToolsPanelOpen(true); setIsMainMenuOpen(false); }} className={`w-full text-left px-4 py-2 flex items-center gap-3 hover:bg-gray-800 transition-colors ${activeTool === tool.id ? 'bg-gray-800 text-white' : 'text-gray-300'}`} > <span className={`w-2 h-2 rounded-full bg-current ${tool.color}`}></span> {tool.label} {activeTool === tool.id && <span className="ml-auto text-xs text-blue-400 font-bold">ACTIVE</span>} </button> ))}
                            <div className="border-t border-gray-700 my-2 mx-2"></div>
                            <div className="px-4 py-2 text-xs font-bold text-gray-500 uppercase tracking-wider">Panels</div>
                            {[ { label: 'Report', state: isReportPanelOpen, toggle: () => setIsReportPanelOpen(p => !p), icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-500 hover:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> }, { label: 'Story Mode', state: isPresentationPanelOpen, toggle: () => setIsPresentationPanelOpen(p => !p), icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-500 hover:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg> }, { label: 'History', state: isHistoryPanelOpen, toggle: () => setIsHistoryPanelOpen(p => !p), icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-500 hover:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> }, { label: 'Table', state: isTablePanelOpen, toggle: () => setIsTablePanelOpen(p => !p), icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-500 hover:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M3 14h18m-9-4v8m-7-4h14M4 6h16a2 2 0 012 2v12a2 2 0 01-2 2H4a2 2 0 01-2-2V8a2 2 0 012-2z" /></svg> } ].map((panel, idx) => ( <button key={idx} onClick={() => { panel.toggle(); setIsMainMenuOpen(false); }} className="w-full text-left px-4 py-2 text-gray-300 hover:bg-gray-800 hover:text-white flex items-center justify-between group" > <div className="flex items-center gap-3"> <div className="group-hover:text-blue-400">{panel.icon}</div> {panel.label} </div> {panel.state && <svg className="w-4 h-4 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>} </button> ))}
                        </div>
                    )}
                </div>
                <div className="border-l border-gray-600 h-6 mx-1"></div>
                <button onClick={handleNewModelClick} title="New Model..." className="p-2 rounded-md hover:bg-gray-700 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg></button>
                <button onClick={handleImportClick} title="Open Model..." className="p-2 rounded-md hover:bg-gray-700 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg></button>
                <button onClick={handleDiskSave} title="Save to Disk" className="p-2 rounded-md hover:bg-gray-700 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg></button>
                <div className="border-l border-gray-600 h-6 mx-1"></div>
                <button onClick={handleCopy} title="Copy Selected (Report)" className="p-2 rounded-md hover:bg-gray-700 transition text-gray-400 hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg></button>
                <button onClick={handlePaste} title="Paste (Add to Model)" className="p-2 rounded-md hover:bg-gray-700 transition text-gray-400 hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg></button>
                <div className="border-l border-gray-600 h-6 mx-1"></div>
                <button onClick={() => setIsToolsPanelOpen(p => !p)} title="Toggle Tools Panel" className={`p-2 rounded-md hover:bg-gray-700 transition ${isToolsPanelOpen ? 'bg-gray-700 text-white' : 'text-blue-400'}`}><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.9 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z" /></svg></button>
                <div className="border-l border-gray-600 h-6 mx-1"></div>
                <button onClick={() => setIsFilterPanelOpen(prev => !prev)} title="Filter by Tag" className="p-2 rounded-md hover:bg-gray-700 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg></button>
                <button onClick={handleToggleFocusMode} title={focusButtonTitle()} className="p-2 rounded-md hover:bg-gray-700 transition">{focusMode === 'narrow' && (<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="3" /></svg>)}{focusMode === 'wide' && (<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="3" /></svg>)}{focusMode === 'zoom' && (<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="3" /><path strokeLinecap="round" strokeLinejoin="round" d="M2 6V2h4 M22 6V2h-4 M2 18v4h4 M22 18v4h-4" /></svg>)}</button>
                
                <button onClick={() => setIsChatPanelOpen(prev => !prev)} title="AI Assistant" className="p-2 rounded-md hover:bg-gray-700 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg></button>
                <button onClick={() => { setSettingsInitialTab('general'); setIsSettingsModalOpen(true); }} title="Settings" className="p-2 rounded-md hover:bg-gray-700 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-400 hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg></button>
                <button onClick={handleZoomToFit} title="Zoom to Fit" className="p-2 rounded-md hover:bg-gray-700 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 8V4h4M20 8V4h-4M4 16v4h4M20 16v4h-4" /></svg></button>
                <div className="relative" ref={helpMenuRef}>
                <button onClick={() => setIsHelpMenuOpen(p => !p)} title="Help" className="p-2 rounded-md hover:bg-gray-700 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.546-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
                {isHelpMenuOpen && <HelpMenu onClose={() => setIsHelpMenuOpen(false)} onAbout={() => setIsAboutModalOpen(true)} onPatternGallery={() => setIsPatternGalleryModalOpen(true)} />}
                </div>
      ...
      {currentModelId && !isPresenting && (
        <div className={`absolute left-4 z-40 max-w-[90vw] pointer-events-none transition-all duration-500 ease-in-out ${isToolsPanelOpen ? 'top-20 opacity-100' : 'top-4 opacity-0'}`}>
            <div className="flex flex-wrap items-start gap-2 pointer-events-auto">
                <button onClick={() => setIsToolsPanelOpen(false)} className="bg-gray-800 hover:bg-gray-700 border border-gray-600 shadow-lg rounded-lg w-20 flex flex-col items-center justify-center transition-colors h-20 flex-shrink-0 z-20 gap-1" title="Close Tools Panel">
                ...
                <SchemaToolbar schemes={colorSchemes} activeSchemeId={activeSchemeId} onSchemeChange={setActiveSchemeId} activeColorScheme={activeColorScheme} onDefaultRelationshipChange={handleUpdateDefaultRelationship} defaultTags={defaultTags} onDefaultTagsChange={setDefaultTags} elements={elements} isCollapsed={activeTool !== 'schema'} onToggle={() => toggleTool('schema')} onUpdateSchemes={(newSchemes) => setColorSchemes(newSchemes)} />
                <LayoutToolbar linkDistance={layoutParams.linkDistance} repulsion={layoutParams.repulsion} onLinkDistanceChange={(val) => setLayoutParams(p => ({...p, linkDistance: val}))} onRepulsionChange={(val) => setLayoutParams(p => ({...p, repulsion: val}))} onJiggle={() => setJiggleTrigger(prev => prev + 1)} isPhysicsActive={isPhysicsModeActive} onStartAutoLayout={handleStartPhysicsLayout} onAcceptAutoLayout={handleAcceptLayout} onRejectAutoLayout={handleRejectLayout} onExpand={() => handleScaleLayout(1.1)} onContract={() => handleScaleLayout(0.9)} isCollapsed={activeTool !== 'layout'} onToggle={() => toggleTool('layout')} />
                <AnalysisToolbar elements={elements} relationships={relationships} onBulkTag={handleBulkTagAction} onHighlight={handleAnalysisHighlight} onFilter={handleAnalysisFilter} isCollapsed={activeTool !== 'analysis'} onToggle={() => toggleTool('analysis')} isSimulationMode={isSimulationMode} onToggleSimulation={() => setIsSimulationMode(p => !p)} onResetSimulation={() => setSimulationState({})} />
                <ScamperToolbar selectedElementId={selectedElementId} onScamper={(operator, letter) => { setScamperInitialDoc(null); setScamperTrigger({ operator, letter }); setIsScamperModalOpen(true); }} isCollapsed={activeTool !== 'scamper'} onToggle={() => toggleTool('scamper')} onOpenSettings={() => { setSettingsInitialTab('tool_prompts'); setIsSettingsModalOpen(true); }} />
                <TrizToolbar activeTool={activeTrizTool} onSelectTool={handleTrizToolSelect} isCollapsed={activeTool !== 'triz'} onToggle={() => toggleTool('triz')} onOpenSettings={() => { setSettingsInitialTab('tool_prompts'); setIsSettingsModalOpen(true); }} />
                <LssToolbar activeTool={activeLssTool} onSelectTool={handleLssToolSelect} isCollapsed={activeTool !== 'lss'} onToggle={() => toggleTool('lss')} onOpenSettings={() => { setSettingsInitialTab('tool_prompts'); setIsSettingsModalOpen(true); }} />
                <TocToolbar activeTool={activeTocTool} onSelectTool={handleTocToolSelect} isCollapsed={activeTool !== 'toc'} onToggle={() => toggleTool('toc')} onOpenSettings={() => { setSettingsInitialTab('tool_prompts'); setIsSettingsModalOpen(true); }} />
                <SsmToolbar activeTool={activeSsmTool} onSelectTool={handleSsmToolSelect} isCollapsed={activeTool !== 'ssm'} onToggle={() => toggleTool('ssm')} onOpenSettings={() => { setSettingsInitialTab('tool_prompts'); setIsSettingsModalOpen(true); }} />
                <SwotToolbar activeTool={activeSwotTool} onSelectTool={handleSwotToolSelect} isCollapsed={activeTool !== 'swot'} onToggle={() => toggleTool('swot')} onOpenSettings={() => { setSettingsInitialTab('tool_prompts'); setIsSettingsModalOpen(true); }} />
                <ExplorerToolbar onSelectTool={handleExplorerToolSelect} isCollapsed={activeTool !== 'explorer'} onToggle={() => toggleTool('explorer')} />
                <TagCloudToolbar onSelectTool={handleTagCloudToolSelect} isCollapsed={activeTool !== 'tagcloud'} onToggle={() => toggleTool('tagcloud')} />
                <MermaidToolbar onSelectTool={handleMermaidToolSelect} isCollapsed={activeTool !== 'mermaid'} onToggle={() => toggleTool('mermaid')} />
                <BulkEditToolbar activeColorScheme={activeColorScheme} tagsToAdd={bulkTagsToAdd} tagsToRemove={bulkTagsToRemove} onTagsToAddChange={setBulkTagsToAdd} onTagsToRemoveChange={setBulkTagsToRemove} isActive={isBulkEditActive} onToggleActive={() => setIsBulkEditActive(p => !p)} isCollapsed={activeTool !== 'bulk'} onToggle={() => toggleTool('bulk')} />
                <CommandBar onExecute={handleCommandExecution} isCollapsed={activeTool !== 'command'} onToggle={() => toggleTool('command')} onOpenHistory={handleOpenCommandHistory} />
            </div>
        </div>
      )}

      {isFilterPanelOpen && currentModelId && !isPresenting && <FilterPanel allTags={allTags} tagCounts={tagCounts} tagFilter={tagFilter} dateFilter={dateFilter} onTagFilterChange={setTagFilter} onDateFilterChange={setDateFilter} onClose={() => setIsFilterPanelOpen(false)} />}
      
      {currentModelId && !isPresenting && <RightPanelContainer panels={panelDefinitions} layouts={panelLayouts} onLayoutChange={setPanelLayouts} activeDockedId={activeDockedPanelId} onActiveDockedIdChange={setActiveDockedPanelId} globalZIndex={panelZIndex} onGlobalZIndexChange={setPanelZIndex} />}
      ...
      {currentModelId && !isPresenting && ((panelState.view === 'addRelationship' && addRelationshipSourceElement) || selectedRelationship || selectedElement) && (
        <div ref={panelRef} className={`z-[70] flex flex-col pointer-events-none ${detailsPanelPosition ? 'fixed shadow-2xl rounded-lg' : 'absolute top-24'}`} style={detailsPanelPosition ? { left: detailsPanelPosition.x, top: detailsPanelPosition.y, maxHeight: 'calc(100vh - 2rem)' } : { right: isRightPanelOpen ? '620px' : '16px', maxHeight: 'calc(100vh - 8rem)' }}>
            <div className="pointer-events-auto flex flex-col h-auto max-h-full shadow-2xl rounded-lg bg-gray-800 border border-gray-700 min-h-0">
                <div className="h-6 bg-gray-700 rounded-t-lg flex items-center justify-center cursor-move border-b border-gray-600 group relative flex-shrink-0" onMouseDown={handlePanelDragStart}>
                    <div className="w-10 h-1 bg-gray-500 rounded-full group-hover:bg-gray-400 transition-colors"></div>
                    <button onClick={handleResetPanelPosition} className="absolute right-2 text-gray-400 hover:text-white" title={detailsPanelPosition ? "Dock Panel" : "Unpin/Float Panel"}>{detailsPanelPosition ? (<svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M16 12V4H8v8L6 14v2h5v6l1 2 1-2v-6h5v-2l-2-2z" /></svg>) : (<svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 transform rotate-45" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16 12V4H8v8L6 14v2h5v6l1 2 1-2v-6h5v-2l-2-2z" /></svg>)}</button>
                </div>
                <div className="rounded-b-lg overflow-hidden flex flex-col min-h-0 flex-grow">
                    {panelState.view === 'addRelationship' && addRelationshipSourceElement ? ( <AddRelationshipPanel sourceElement={addRelationshipSourceElement} targetElementId={panelState.targetElementId} isNewTarget={panelState.isNewTarget} allElements={elements} onCreate={handleAddRelationship} onUpdateElement={handleUpdateElement} onCancel={handleCancelAddRelationship} suggestedLabels={activeRelationshipLabels} defaultLabel={activeColorScheme?.defaultRelationshipLabel} colorSchemes={colorSchemes} activeSchemeId={activeSchemeId} /> ) : selectedRelationship ? ( <RelationshipDetailsPanel relationship={selectedRelationship} elements={elements} onUpdate={handleUpdateRelationship} onDelete={handleDeleteRelationship} suggestedLabels={activeRelationshipLabels} /> ) : selectedElement ? ( <ElementDetailsPanel element={selectedElement} allElements={elements} relationships={relationships} onUpdate={handleUpdateElement} onDelete={handleDeleteElement} onClose={() => setSelectedElementId(null)} colorSchemes={colorSchemes} activeSchemeId={activeSchemeId} /> ) : null}
                </div>
            </div>
        </div>
    )}
      ...
      <ChatPanel
          ...
          onOpenPromptSettings={() => { setSettingsInitialTab('ai_prompts'); setIsSettingsModalOpen(true); }}
      ...
      <ScamperModal isOpen={isScamperModalOpen} onClose={() => setIsScamperModalOpen(false)} elements={elements} relationships={relationships} selectedElementId={selectedElementId} modelActions={aiActions} triggerOp={scamperTrigger} onClearTrigger={() => setScamperTrigger(null)} documents={documents} folders={folders} onUpdateDocument={handleUpdateDocument} modelName={currentModelName} initialDoc={scamperInitialDoc} onLogHistory={handleLogHistory} defaultTags={defaultTags} aiConfig={aiConfig} />
      <TrizModal isOpen={isTrizModalOpen} activeTool={activeTrizTool} elements={elements} relationships={relationships} modelActions={aiActions} documents={documents} folders={folders} onUpdateDocument={handleUpdateDocument} initialParams={trizInitialParams} onClose={() => setIsTrizModalOpen(false)} onLogHistory={handleLogHistory} onOpenHistory={() => setIsHistoryPanelOpen(true)} onAnalyze={handleAnalyzeWithChat} customPrompt={getToolPrompt('triz', activeTrizTool)} aiConfig={aiConfig} />
      <LssModal isOpen={isLssModalOpen} activeTool={activeLssTool} elements={elements} relationships={relationships} modelActions={aiActions} documents={documents} folders={folders} onUpdateDocument={handleUpdateDocument} initialParams={lssInitialParams} onClose={() => setIsLssModalOpen(false)} onLogHistory={handleLogHistory} onOpenHistory={() => setIsHistoryPanelOpen(true)} onAnalyze={handleAnalyzeWithChat} customPrompt={getToolPrompt('lss', activeLssTool)} aiConfig={aiConfig} />
      <TocModal isOpen={isTocModalOpen} activeTool={activeTocTool} elements={elements} relationships={relationships} modelActions={aiActions} documents={documents} folders={folders} onUpdateDocument={handleUpdateDocument} initialParams={tocInitialParams} onClose={() => setIsTocModalOpen(false)} onLogHistory={handleLogHistory} onOpenHistory={() => setIsHistoryPanelOpen(true)} onAnalyze={handleAnalyzeWithChat} customPrompt={getToolPrompt('toc', activeTocTool)} aiConfig={aiConfig} />
      <SsmModal isOpen={isSsmModalOpen} activeTool={activeSsmTool} elements={elements} relationships={relationships} modelActions={aiActions} documents={documents} folders={folders} onUpdateDocument={handleUpdateDocument} initialParams={ssmInitialParams} onClose={() => setIsSsmModalOpen(false)} onLogHistory={handleLogHistory} onOpenHistory={() => setIsHistoryPanelOpen(true)} onAnalyze={handleAnalyzeWithChat} customPrompt={getToolPrompt('ssm', activeSsmTool)} aiConfig={aiConfig} />
      <SwotModal isOpen={isSwotModalOpen} activeTool={activeSwotTool} elements={elements} relationships={relationships} modelActions={aiActions} documents={documents} folders={folders} onUpdateDocument={handleUpdateDocument} onClose={() => setIsSwotModalOpen(false)} onLogHistory={handleLogHistory} onOpenHistory={() => setIsHistoryPanelOpen(true)} modelName={currentModelName} initialDoc={swotInitialDoc} aiConfig={aiConfig} />
      <SettingsModal isOpen={isSettingsModalOpen} onClose={() => setIsSettingsModalOpen(false)} initialTab={settingsInitialTab} globalSettings={globalSettings} onGlobalSettingsChange={handleGlobalSettingsChange} modelSettings={systemPromptConfig} onModelSettingsChange={setSystemPromptConfig} aiConfig={aiConfig} onAiConfigChange={handleAiConfigChange} />
      {isSchemaUpdateModalOpen && <SchemaUpdateModal changes={schemaUpdateChanges} onClose={() => setIsSchemaUpdateModalOpen(false)} />}
        ...
        <div className="w-full h-full flex-col items-center justify-center bg-gray-900 text-white space-y-10 p-8 flex">
             ...
             <div className="flex space-x-8">
                <button onClick={() => setIsCreateModelModalOpen(true)} className="flex flex-col items-center justify-center w-56 h-56 bg-gray-800 border-2 border-gray-700 rounded-2xl hover:border-blue-500 hover:bg-gray-750 hover:scale-105 hover:shadow-lg hover:shadow-blue-500/20 transition-all group">
                    <div className="bg-gray-700 rounded-full p-4 mb-4 group-hover:bg-blue-900 group-hover:bg-opacity-30 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-gray-400 group-hover:text-blue-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg></div>
                ...
                <button onClick={handleImportClick} className="flex flex-col items-center justify-center w-56 h-56 bg-gray-800 border-2 border-gray-700 rounded-2xl hover:border-green-500 hover:bg-gray-750 hover:scale-105 hover:shadow-lg hover:shadow-green-500/20 transition-all group">
                     <div className="bg-gray-700 rounded-full p-4 mb-4 group-hover:bg-green-900 group-hover:bg-opacity-30 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-gray-400 group-hover:text-green-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg></div>
             ...
             {modelsIndex.length > 0 && (
                 <div className="mt-8 w-full max-w-2xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                     ...
                     <div className="text-center mt-4"><button onClick={() => setIsOpenModelModalOpen(true)} className="text-sm text-blue-400 hover:text-blue-300 hover:underline">View All Recovered Models</button></div>
                 </div>
             )}
        </div>
      )}

      {contextMenu && currentModelId && <ContextMenu x={contextMenu.x} y={contextMenu.y} onClose={handleCloseContextMenu} onAddRelationship={() => { setPanelState({ view: 'addRelationship', sourceElementId: contextMenu.elementId, targetElementId: null, isNewTarget: false }); setSelectedElementId(null); setMultiSelection(new Set()); setSelectedRelationshipId(null); handleCloseContextMenu(); }} onDeleteElement={() => { handleDeleteElement(contextMenu.elementId); handleCloseContextMenu(); }} />}
      {canvasContextMenu && currentModelId && <CanvasContextMenu x={canvasContextMenu.x} y={canvasContextMenu.y} onClose={handleCloseCanvasContextMenu} onZoomToFit={handleZoomToFit} onAutoLayout={handleStartPhysicsLayout} onToggleReport={() => setIsReportPanelOpen(p => !p)} onToggleMarkdown={() => setIsMarkdownPanelOpen(p => !p)} onToggleJSON={() => setIsJSONPanelOpen(p => !p)} onToggleFilter={() => setIsFilterPanelOpen(p => !p)} onToggleMatrix={() => setIsMatrixPanelOpen(p => !p)} onToggleTable={() => setIsTablePanelOpen(p => !p)} onToggleGrid={() => setIsGridPanelOpen(p => !p)} onOpenModel={handleImportClick} onSaveModel={handleDiskSave} onCreateModel={handleNewModelClick} onSaveAs={() => setIsSaveAsModalOpen(true)} isReportOpen={isReportPanelOpen} isMarkdownOpen={isMarkdownPanelOpen} isJSONOpen={isJSONPanelOpen} isFilterOpen={isFilterPanelOpen} isMatrixOpen={isMatrixPanelOpen} isTableOpen={isTablePanelOpen} isGridOpen={isGridPanelOpen} />}
      {isCreateModelModalOpen && <CreateModelModal onCreate={handleCreateModel} onClose={() => setIsCreateModelModalOpen(false)} isInitialSetup={!modelsIndex || modelsIndex.length === 0} />}
      {isSaveAsModalOpen && <SaveAsModal currentName={modelsIndex.find(m => m.id === currentModelId)?.name || ''} currentDesc={modelsIndex.find(m => m.id === currentModelId)?.description || ''} onSave={handleSaveAs} onClose={() => setIsSaveAsModalOpen(false)} />}
      ...
      {pendingImport && <ConflictResolutionModal localMetadata={pendingImport.localMetadata} diskMetadata={pendingImport.diskMetadata} localData={pendingImport.localData} diskData={pendingImport.diskData} onCancel={() => setPendingImport(null)} onChooseLocal={() => { handleLoadModel(pendingImport.localMetadata.id); setPendingImport(null); }} onChooseDisk={() => { loadModelData(pendingImport.diskData, pendingImport.diskMetadata.id, pendingImport.diskMetadata); setPendingImport(null); }} />}
